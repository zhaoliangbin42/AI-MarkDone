...
正式 conversation identity 为当前页面纪元打开一次性 baseline gate。第一个身份匹配且可验证的 Graph 会建立初始 cache；正在生成的末尾 assistant 不进入 cache，等 DOM 完成后由 Host Monitor 追加。同一纪元后续 capture、消费者 `refresh()`、route hash 与普通 DOM mutation 都不能重开或重读 baseline。若 document-start bridge 错过首次官网响应，已有长对话必须刷新页面重新捕获。空白页首轮例外由 typed DOM `empty-proven`、临时 `WEB` 路由/birth 事实与稳定 host-born turn 建立，不依赖域名或 `/` 路径。
