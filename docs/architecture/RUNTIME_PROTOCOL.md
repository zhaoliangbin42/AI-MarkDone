...
每个 content Runtime 先建立一个仅存在于当前页面内存的 page identity；稳定 DOM 批次可直接建立 `host` pool，不等待 URL 或 conversation ID。正式 canonical identity 后到时只 promotion 同一 projection：turns、正文、projection ID 与 content token 不变，同时为该 canonical epoch 打开一次性 baseline gate。合法 Graph 先到时建立 `source` pool；DOM 先到后，首个可信 Graph 只有与现有首条身份可靠重合时，才可一次性补入该锚点之前的历史前缀并形成 `hybrid` pool。已有正文、digest 与 assistant identity 永不覆盖。无重合、顺序或身份冲突的 Graph 被忽略，不破坏当前 pool。同一 epoch gate 关闭后的 capture、消费者 `refresh()`、hash 与普通 DOM mutation 都不能重开或重读 baseline。Baseline 失败也不能把已有 host-ready snapshot 降级。

唯一 `ChatGPTPageIndex` observer 在 content runtime 内输出带 page URL、surface epoch、typed identity、完整 round、generation start/end、同 owner assistant identity replacement、root rebase 与 assistant-only materialization 的 host facts；这些事实允许 React persistent slot 在多个 mutation batch 中逐步水合，不要求同批“出生”。`ChatGPTConversationHostMonitor` 只记录 dirty assistant identity，先以唯一挂载的缓存尾部证明顺序；尾部未挂载时，真实 generation start 或缓存尾部的同 owner replacement 才能把候选锚定到缓存尾部。候选位于可见尾部之前、跨越未解析 round 或顺序有歧义时暂缓。正文退出 streaming、非空且 DOM 安静 400 ms 后，官方 action anchor、generation end 或后续 typed round 任一成立即可进入 clone/compile；都不存在时等待总计 2 秒的有界安静确认。稳定轮次按一个窗口原子提交到同一 Content Session，action anchor 只继续约束工具栏挂载。逐字符 content mutation 不触发 Surface 重建或 PageIndex topology 重扫；root rebase 会 fence 旧编译并清除瞬态 generation/replacement evidence；ChatGPT hydration 移除带统一 consumer marker 的 Directory、Stepper 或 Toolbar host 时，同一个 observer 发布 surface-only fact，由现有消费者从当前 Frame 自愈，不重读正文、不改变 content token。该 DOM seam 不跨 content/background runtime，不新增协议消息，也不产生网络请求。`ChatGPTConversationContentRuntime` 是唯一顺序化入口：先同步 page/canonical identity 并 fence 旧任务，再处理 PageIndex 或 Graph evidence，最后由 `ChatGPTConversationSurface` 原子连接 Repository 与当前 DOM。Runtime 直接处理初始化、`history.pushState`、`history.replaceState`、`popstate`、`hashchange`、`pageshow` 和 Graph signal；内容链路没有轮询 RouteWatcher。Graph signal 必须先同步当前 identity 再唤醒 gate。request/response 在 Chrome 使用 object detail、Firefox 使用 JSON-string detail；capture event 在两者都使用 JSON string。两种 transport 必须产生相同的 baseline admission 与 `ConversationContentSourceV1` 状态序列。
