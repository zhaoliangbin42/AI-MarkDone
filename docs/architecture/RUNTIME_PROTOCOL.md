...
- capture event: `aimd:chatgpt-conversation-bridge:capture` 只作失效/重调度信号
  - 当前唯一合法 kind 是 `"graph"`，表示 Bridge 内存中出现新的、已通过基础结构验证的 conversation graph；只有当前 conversation epoch 尚未关闭的 baseline gate 可以消费

桥边界固定为：只观察官网自身成功的 same-origin `GET`，不观察 POST 或 `PerformanceObserver`，不读取或传播 cookie、token、Authorization/session header、请求体或 SSE，也不持久化正文。候选 URL 的已解码 path/query token 必须精确包含当前 canonical conversation ID；响应必须声明 JSON Content-Type，并在最多 4 层、256 个普通对象的有界遍历内发现 `mapping + current_node` Graph。payload 如果声明 conversation ID 必须与当前 ID 一致；active branch、parent 链、角色、消息 ID 与完整 user/assistant round 仍由 Adapter/Repository 校验。bridge 用透明 `window.fetch` 包装观察并对已返回 response 做 `clone().json()`，官网收到的原请求和原响应保持不变。它不依赖任何固定 endpoint 模板；content runtime 的所有 `peek()` 都是内存读取，不发起网络请求。

每个 content Runtime 先建立一个仅存在于当前页面内存的 page identity；稳定 DOM 批次可直接建立 `host` pool，不等待 URL 或 conversation ID。正式 canonical identity 后到时只 promotion 同一 projection：turns、正文、projection ID 与 content token 不变，同时为该 canonical epoch 打开一次性 baseline gate。合法 Graph 先到时建立 `source` pool；DOM 先到后，首个或后续可信 Graph 都必须包含当前池全部 typed identity 并保持相对顺序，通过后按 Graph 的完整顺序汇合，补入隐藏 prefix、middle 与 tail，已有 strong 正文、digest 与 assistant identity 永不覆盖，只有 ADR-0019 的 weak-sealed 正文可由更强 Graph 证据升级。无重合、缺失既有 turn、顺序或身份冲突、分支替换或 proof 不完整的 Graph 被忽略，不破坏当前 pool。同一 epoch gate 关闭后的真实 capture 可以触发一次有界 upgrade comparison；`pageshow`/Settings retry 只能武装一次被动 upgrade peek，消费者 `refresh()`、hash 与普通 DOM mutation 都不能主动读取或重开 baseline。Baseline 失败也不能把已有 host-ready snapshot 降级。

唯一 `ChatGPTPageIndex` observer 在 content runtime 内输出带 page URL、surface epoch、typed identity、完整 round、generation start/end、同 owner assistant identity replacement、root rebase 与 assistant-only materialization 的 host facts；这些事实允许 React persistent slot 在多个 mutation batch 中逐步水合，不要求同批“出生”。PageIndex 以 wrapper 发现作为常态快路径；只有它发现存在未覆盖的 assistant surface 时，才合并 legacy-container 与 role-scan 证据，并按挂载 assistant identity/root 去重、按文档顺序重排，避免历史 wrapper 与新增 role-shaped tail 混合时丢失消息。`ChatGPTConversationHostMonitor` 只记录 dirty assistant identity，先以唯一挂载的缓存尾部证明顺序；尾部未挂载时，真实 generation start 或缓存尾部的同 owner replacement 才能把候选锚定到缓存尾部。候选位于可见尾部之前、跨越未解析 round 或顺序有歧义时暂缓。正文退出 streaming、非空且 DOM 安静 400 ms 后，官方 action anchor、generation end 或后续 typed round 任一成立即可进入 clone/compile；都不存在时等待总计 2 秒的有界安静确认。稳定轮次按一个窗口原子提交到同一 Content Session，官方 action row 只作为生命周期/完成证据与工具栏挂载约束，不能作为正文来源。逐字符 content mutation 不触发 Surface 重建或 PageIndex topology 重扫；root rebase 会 fence 旧编译并清除瞬态 generation/replacement evidence；ChatGPT hydration 移除带统一 consumer marker 的 Directory、Stepper 或 Toolbar host 时，同一个 observer 发布 surface-only fact，由现有消费者从当前 Frame 自愈，不重读正文、不改变 content token。该 DOM seam 不跨 content/background runtime，不新增协议消息，也不产生网络请求。`ChatGPTConversationContentRuntime` 是唯一顺序化入口：先同步 page/canonical identity 并 fence 旧任务，再处理 PageIndex 或 Graph evidence，最后由 `ChatGPTConversationSurface` 原子连接 Repository 与当前 DOM。Runtime 直接处理初始化、`history.pushState`、`history.replaceState`、`popstate`、`hashchange`、`pageshow` 和 Graph signal；内容链路没有轮询 RouteWatcher。Graph signal 必须先同步当前 identity 再唤醒 gate。request/response 在 Chrome 使用 object detail、Firefox 使用 JSON-string detail；capture event 在两者都使用 JSON-string detail。两种 transport 必须产生相同的 baseline admission 与 `ConversationContentSourceV1` 状态序列。

若 canonical identity 尚未出现，稳定 typed rounds 以 page identity 直接发布并供工具栏、字数、Directory、Stepper、Reader、复制、公式与导出消费。因为现有书签协议需要 canonical conversation ID 与 URL，此时书签准备直接不可用，且不得调用保存/删除接口或构造缺字段记录；identity promotion 后沿用原协议自动恢复。canonical A→B 立即创建新 projection、清空旧 staging 并使旧异步编译/导航/选择失效。无 ID 且 URL 始终不变的匿名会话属于当前支持范围，但 page identity 不持久化、不跨页面导航，也不能凭空恢复未被 Graph 或 DOM 暴露的隐藏历史。

本页内容与同页导航的异步边界由 Runtime/Repository 发布的 projection identity 决定，不由 URL 字符串决定。URL 不变时后续稳定 DOM turn 必须继续进入当前 pool；query/hash/路由文字变化但 projection 不变时不能清空内容或取消定位。旧 Discovery Coordinator、Conversation Index 与独立 Materialization 模块已删除，协议层不得为它们新增事件、请求或兼容状态。

虚拟化窗口只剩 assistant root 时，`ChatGPTPageIndex` 输出的是既有缓存消息的 assistant-only surface fact；它只更新 `ChatGPTConversationSurface`、工具栏和当前视口几何，不进入 Host Monitor 的新消息 admission，不触发 bridge replay、baseline 重开或 content token 变化。user root 重新挂载后，PageIndex 以同一 typed assistant identity 幂等替换当前表面投影。Surface 的兼容 `ConversationMaterializationPortV1` 从同一 Frame 派生，不维护第二份状态。
