...
ChatGPT production content has one composition root, one accumulated pool, one
PageIndex-owned mounted-message seam, and one 5.3-compatible source-seed seam:

```text
Website-owned GET -> document-start bridge -> GET seed (`get`)
                                   |
...
- Ordinary consumer actions read the current published cache through that projection. `refresh()` is a local DOM flush compatibility seam; it cannot issue a request or discover unloaded history. The Runtime may perform the single 5.3-compatible bridge-memory `readBaseline()` for the current epoch; it observes only a website-owned GET and publishes `get`, never an extension conversation request. `pageshow`, `resume`, and visible restoration may request one coalesced current-DOM rescan. There is no Settings Retry or polling loop.
...
- ChatGPT discovery must not read Cookie/Storage/Token/Authorization data, construct conversation requests, use React internals, or poll content. The document-start bridge may inspect only bounded successful same-origin JSON GET clones matching the active conversation and may not inspect POST/SSE. The explicit `?message=` route only creates the official navigation skeleton; page entry must not run a whole-history slot sweep. User-facing `locate()` may use bounded single-target seek. Neither path creates a second observer, source, or permanent auto-scroll.
...
- `ConversationSnapshotV1.coverage` remains `complete` for admitted bodies; ordinary DOM-only snapshots report `historyStatus: partial`, a 5.3 source seed reports `historyStatus: get`, and `proof.basis` identifies the current mix. The current runtime does not manufacture `historyStatus: complete` through page-entry scrolling; new topology returns an existing proven snapshot to `get` or `partial` according to source availability. Current-message and selection actions may compile only their mounted owning DOM; multi-message consumers must use the Repository snapshot and cannot rebuild an independent pool.
