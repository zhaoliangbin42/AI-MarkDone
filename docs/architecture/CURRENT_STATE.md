...
- ChatGPT 当前的专属增强能力已经改成 **payload/store-first**：
  ...
  - `ChatGPTPageIndex` 是 ChatGPT 当前页面 DOM 轮次/锚点投影的唯一 revision cache。它在 adapter/driver 层为每次相关宿主 DOM revision 只构建一份有序 `ChatGPTDomRoundRef[]`，并让 adapter group refs 与 `collectConversationTurnRefs()` 的映射结果按对象身份复用；工具栏、目录条、lower-right stepper、发送位置恢复和同页定位不得另做整页轮次扫描。宿主 child/text/identity attribute 变化会失效；AI-MarkDone toolbar 插入与 `data-aimd-*` bookkeeping 必须被过滤，避免自失效；conversation root 被替换时自动重绑，runtime disable 时由 adapter `dispose()` 断开 observer 并清空缓存。该索引只包含当前 DOM 的位置/锚点，不替代 `ChatGPTConversationEngine` 的完整正文 snapshot。
...
- Settings tab 的主顺序是 Platforms、Buttons & Entrypoints、ChatGPT Reading & Input、Reader & Comment Workflow、Copy, Formula & Export、Language、Data Management、Advanced。Buttons & Entrypoints 是无路由、非持久化的二级页，集中持有 `behavior.showMessageToolbar`、`behavior.showSaveMessages`、`behavior.showWordCount`、`chatgptBehavior.showPageBookmarkControl`、`chatgptBehavior.showDetachedReaderControl`、`chatgptBehavior.showPromptControl`、`chatgptBehavior.showMessageStepper` 和公式 hover action 可见性。ChatGPT Reading & Input 持有 `chatgptBehavior.restorePositionAfterSend` 发送后恢复阅读位置开关、`chatgptBehavior.enterKeyNewline` 输入框普通 Enter 换行开关、`chatgptBehavior.promptAutocomplete` 输入 `\` 自动显示 Prompt 候选开关、`chatgptBehavior.pageWidthScale` 对话宽度滑块、`chatgptBehavior.enableArrowKeyMessageNavigation` 左右方向键切换消息开关，以及 `chatgptDirectory` 目录条开关、显示模式、prompt label 模式、右侧边距滑块；`chatgptBehavior.enterKeyNewline` 开启后普通 Enter 换行，发送保留 ChatGPT 发送按钮和 Cmd/Ctrl + Enter；`chatgptBehavior.promptAutocomplete` 默认开启，关闭后不影响手动 Prompt manager 入口；`chatgptBehavior.pageWidthScale` 默认 100，可在 100%–200% 内调大 ChatGPT 对话区域；四个 lower-right 按钮开关默认开启并只影响对应按钮；`chatgptDirectory.rightInsetPx` 默认 0px，可在 0–40px 内用于浏览器滚动条覆盖目录条时手动增加额外边距；`chatgptDirectory.enabled` 默认开启，不复用平台总开关或旧 `chatgpt` category，且启用目录条时同步隐藏 ChatGPT 官方对话导航，不再暴露独立隐藏开关
...
- ChatGPT 官网正文直选是一条严格限定的 selection exit，不是新的消息正文来源：`ChatGPTAtomicSelectionController` 只在同一条非流式 assistant `.markdown.prose` 内读取当前单 Range，用一个 document `selectionchange` listener + `requestAnimationFrame` 识别被完整覆盖的 rendered atomic unit，并仅对 selected-set 差分写入 tokenized `data-aimd-page-atomic-state`。复制时才在 32ms/5000-node 总预算内把每个已知原子节点送入现有 adapter 的 clone/normalize/noise-removal/parser/cleanMarkdown 链，再与普通可见文本切片重建 canonical Markdown；公式必须复用共享 `extractLatexSource` 的可逆 TeX signal，不能从视觉文本猜测。该 canonical 输出保持源码语义，但不承诺恢复宿主未暴露的后端原始空白或字节串。严格原子清洗成功后，clipboard exit 会覆盖 ChatGPT 已 `preventDefault` 写入的视觉文本但不停止事件传播；普通/部分选区、跨消息、user/composer/control/custom widget、不可逆源码、超预算和任意 DOM/parser/clipboard 失败全部 fail open。该 controller 不建立 MutationObserver、逐消息 listener、overlay、选区改写或源码缓存，也不能被 Reader、toolbar、bookmark、Save Messages 或 export 当作 `ReaderItem[]` 替代链路。
...
- 公式点击复制与单公式 PNG/SVG/MathML hover 动作由 `FormulaAssetHoverController` 统一承载，运行时消费 `formula` settings、`platforms` settings 与 build-time target surface policy 做 gating；hover 动作默认全部关闭，只有用户显式启用的 action 才显示，既有已存 `formula.assetActions` 选择在归一化/迁移中保留。ChatGPT 通过完整 message runtime 启用该 controller；Gemini、Claude、DeepSeek 通过 formula runtime 在 bounded formula roots 上启用同一个 controller，并把恢复的旧 `MarkdownParserAdapter` 公式子链路注入 `MathClickHandler`，由平台自己的 `isMathNode`、`extractLatex` 与 `isBlockMath` 完成识别、源码提取和 display mode 判定。Formula runtime 可由对应平台开关实时关闭；它会参与 `content:ready` / `ping` / `ui:toggle_toolbar` 运行时协议，使扩展图标保持 active 并可打开全局 BookmarksPanel。点击单公式复制由 `formula.clickCopyFormulaFormat` 控制，整段源码输出由 `formula.markdownCopyFormulaFormat` 控制；两个入口共享 5 个格式：Markdown dollar、LaTeX brackets、raw LaTeX、`equation`、`equation*`。整段 Markdown 的公式替换由 `src/core/math/formulaSourceFormat.ts` 基于 `remark-parse` + `remark-gfm` + `remark-math` 的 AST position 执行，不使用全局正则；它必须避开 code span/fence 等非公式内容，表格内强制 inline-safe 输出，parser 失败时回退原 Markdown。Toolbar Copy Markdown、Reader Copy、Reader 选区源码复制、Detached Reader copy、消息书签 Markdown copy 与 Save Messages Markdown 下载只在写入剪贴板或 Markdown 文件前重写公式 wrapper，Reader 内部 canonical Markdown、PDF/PNG 渲染、书签存储和单公式 PNG/SVG/MathML 导出不受影响。Safari App Store target 隐藏二进制 PNG/SVG clipboard copy 动作，但保留 MathML copy 与 PNG/SVG save 下载动作。单公式 PNG 继续优先捕获页面已渲染 DOM 到 isolated clone，并在失败时回退到 MathJax SVG rasterize；单公式 SVG 始终从同一份 LaTeX source 进入 isolated MathJax renderer，生成显式 glyph path + 标准 CJK `<text>` 的独立 SVG，不允许 `foreignObject`、字体 payload 或外部资源，也不允许失败后回退到 DOM-SVG/PNG。SVG 保持自然比例与完整横向/纵向尺寸，不换行、不截断、不做非等比压缩；renderer 只启用固定安全 TeX package allowlist，未知命令和 MathJax error 明确返回 render failure。MathML 始终由同一 renderer 输出 Presentation MathML。LaTeX source 提取、PNG DOM capture、MathJax renderer、PNG rasterize、clipboard/download services 仍保持独立，不感知 Settings UI
- ChatGPT 完整 runtime 的 `MathClickHandler` 对所有已启用 message container 只建立一个 document-level `MutationObserver`，新增节点先按已启用容器过滤，再通过单次合并 selector 扫描公式候选；重复 enable 不得重扫，公式交互 gate 未变化的 settings update 不得 teardown/re-enable。真正离开文档的 message 必须释放公式 listener 与容器引用，同页 reparent 的 message 则继续保持公式交互。
