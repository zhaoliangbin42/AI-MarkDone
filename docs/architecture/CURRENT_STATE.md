...
> **ChatGPT discovery current boundary (2026-08-10)**
>
> The ChatGPT production runtime uses one baseline-and-tail content session. The document-start MAIN-world bridge passively observes the website's own same-origin conversation GET and can establish one complete history prefix per conversation epoch. After that once-only gate closes, `ChatGPTConversationHostMonitor` consumes typed lifecycle facts from the single `ChatGPTPageIndex` observer, waits for a 400 ms stable window, compiles only the new completed DOM turn, and appends it to the immutable Repository projection. Directory, Reader, Bookmark, Copy, Formula, Word Count and Export all consume that same `ConversationContentRepository` instance; toolbars subscribe to shared materialization and read the exact turn by assistant identity. The bridge never issues a conversation request or reads cookies/storage/tokens/auth headers. See [ADR-0017](../adr/ADR-0017-chatgpt-baseline-and-host-tail-lifecycle.md).

> Message-bookmark highlighting is a read-only projection over the same canonical source. `conversationBookmarkResolver` matches persisted assistant `messageId` first, validates the stored position when the identity is present, and only falls back to position for identity-less or source-proven legacy records. An identity/position conflict fails closed. It never rewrites a `Bookmark`, changes a storage key, migrates old data, or changes import/export shape.

> **ChatGPT consumer wiring audit (2026-08-10)**
>
> The production composition root now has one explicit ChatGPT consumer graph: passive Baseline Adapter + shared Page Monitor/Host Adapter → `ConversationContentRepository`/`ConversationContentSourceV1` → `ChatGPTConversationIndex` and `ConversationMaterializationPortV1`, with `ConversationNavigationPortV1` shared by Directory, Bookmarks Go, Reader locate, Stepper and pending restore. Reader/word count, whole-message copy, Bookmark Preparation and Save Messages read the same source, while precise local selection still requires `SurfaceProjection` source-span proof. The ChatGPT message toolbar owns neither a MutationObserver nor a route watcher; non-ChatGPT platforms retain their local observer path. Persisted bookmark shape, keys, old records and import/export remain unchanged. Automated gates do not replace the separate installed Chrome/Firefox acceptance requirement.
...
- ChatGPT 内容发现采用 **一次被动 Graph 基线 + 稳定 DOM 尾部 + Conversation Content Port V1 + fail-closed**。生产 content runtime 将 `ChatGPTConversationDiscoveryAdapter` 的被动 `peek()` 作为每个 conversation epoch 的一次性基线输入；gate 关闭后，同一会话的 bridge capture 和消费者 `refresh()` 都不会再次读取或改写基线。`ChatGPTConversationHostMonitor` 订阅唯一 `ChatGPTPageIndex` observer，只把 typed identity 完整、退出 streaming、拥有官方操作锚点、DOM 安静 400 ms 且通过 `RenderedContentCompilerV2` 校验的新尾部提交给 Repository。document-start bridge 只回放已观察到的官网自有 conversation GET；扩展主动 conversation GET/POST 恒为零，也不提供 timer-based 网络恢复。
  - V1 的 `ConversationContentSourceV1` 只发布不可变 `idle/syncing/ready/stale/unavailable` 状态、`projectionId` 和 `contentToken`；proof 增加 `basis: source | hybrid | host-born`。每条 assistant Markdown 带 `{ authority, fidelity, producer }` provenance；DOM 尾部固定为 `host-rendered / normalized / rendered-content-v2`。`ConversationMaterializationPortV1` 只暴露当前 DOM anchor 与精确 target，不能输出正文、顺序或 provider payload。Reader、Copy、Save Messages、Bookmark 从 V1 snapshot 投影；Directory/Stepper 使用 V1 + materialization；精确 selection/annotation 仍必须通过 SurfaceProjection；非 ChatGPT 继续 legacy DOM path。
- 当前完整性由 `ConversationSnapshotV1.proof`（`basis/order/bodies/tail/gaps`）决定，`ready/stale/unavailable` 只描述可读生命周期，不把 `ready` 当作全局收敛。桥接 graph 的完整历史前缀可在唯一流式尾部存在时先发布 partial source projection，尾部稳定后由 DOM commit 闭合为 hybrid。相同 typed identity/content 幂等；同一 projection 的 sealed turn 不原地改写。最新 host 后缀重新生成会创建新 `projectionId` 并原子替换后缀；变化触及 baseline prefix 时进入 stale，Reader 与全对话导出暂停直至刷新。`primary/verified-derived + exact/normalized` 才是 source-backed；`host-rendered/normalized` 可用于 whole-turn Reader、Copy、Bookmark、word count 与 export，但不能冒充精确 provider source span。
  ...
  - page bridge 通过 manifest `document_start` 在页面主执行环境安装，只被动旁路观察 ChatGPT 页面自身的 same-origin conversation graph GET；content runtime 的 `peek()` 仅读取 bridge 内存中的最近 evidence。对 generation POST/SSE 只读取同源 URL/method 作为生命周期提示；bridge 不读取请求体、生成正文、Cookie、Token 或认证 header，不 clone generation response，也不构造认证信息。若 document-start bridge 错过官网首次 conversation GET，必须刷新页面重新捕获。
- `ChatGPTPageIndex` 是唯一 Page Monitor observer，监听 conversation/message 结构、identity attributes、语言无关 `data-testid`，以及限定在 typed assistant message 内的 `characterData`/child-list 生命周期变化；扩展自有节点、无关 child-list churn 和本地化 `aria-label` 被忽略。逐字变化只输出 dirty assistant ID。`ChatGPTConversationHostMonitor` 在安静窗口后做一次 typed tail scan/clone/compile，并将验证成功的正文提交给 Repository；它不读取 bridge、不轮询、不创建第二 observer。`ChatGPTDomTurnFactSource` 只保留为无状态兼容投影。
  - Source graph 决定完整历史前缀、branch 与绝对顺序；host 只允许从 source tail 或上一 sealed host tail 连续追加，不能补历史缺口。生产生命周期决策由 `ConversationContentRepository` 统一持有；`ConversationEvidenceLedger` 只保留为 provider-neutral reducer/兼容测试 seam，不构成第二生产仓库。虚拟化缩窗只移除 materialization anchor，sealed 内容与 ordinal 保留；重挂只更新 materialization token，不重新编译或重复挂载工具栏。
  - bridge request/response 在 Chrome 使用 object detail、Firefox 使用 JSON string detail；graph 与 generation capture event 在两种浏览器都使用 JSON string detail。差异只存在于 transport encode/decode 层；Reader、Bookmark、Copy、Save Messages 与 ChatGPT 消息词数通过 `ConversationContentSourceV1` / `readerContentSource` 消费同一个 V1 snapshot，不得新增浏览器分支或消费者自有 DOM fallback。
  - 完成态 `Deep Research` 报告继续属于同一 snapshot SSOT：page bridge 只识别已验证的 Deep Research resource 标识，并从其 `widget_state.report_message` 读取完整 assistant Markdown；未完成、空白、未知或损坏的 widget 必须 fail closed，上传文件正文、工具调用参数与其他 tool 输出不得进入报告。DOM 入口侧只把位于 `data-conversation-screenshot-content` 内的精确 `iframe[title="internal://deep-research"]` 视为已验证消息表面，并把既有消息工具栏挂到报告内容栈底部；iframe 延迟 hydration 复用唯一 PageIndex lifecycle 与共享 Materialization，未知或无报告根的 iframe 不注入。iframe 只提供身份与锚点，报告正文仍复用既有 `normalizeChatGPTReaderMarkdown()` 与 `ReaderItem[]` 链路，不新增 iframe 正文采集、host permission、runtime protocol 或 Deep Research 专属 Reader/导出分支。
  - `ChatGPTPageIndex` 只缓存当前 materialized DOM window 的 connected anchors 与结构信号；它不拥有完整轮次数、绝对顺序或正文。`ChatGPTConversationIndex` 只做 navigation projection：绑定 V1/兼容 source 后直接读取其当前 immutable snapshot，不保存第二份 canonical snapshot，再通过 typed `roundId` / `userMessageId` / `assistantMessageId` 连接 PageIndex anchors；它不发起语义请求、不缓存请求，也不因 unknown identity 强刷。已挂载 assistant message element 的唯一 `data-message-id` 必须直接解析到 canonical `assistantMessageId`，宿主 hydration 产生的 wrapper/turn ID 漂移不得否定这条精确消息身份；缺少直接消息身份时才使用 materialized anchor containment，歧义必须 fail closed。Prompt 文本与 DOM-local position 不得作为 identity；DOM 虚拟窗口变化不得缩短目录或 stepper 总数。PageIndex 必须过滤 AI-MarkDone 自有 mutation，conversation root 替换时重绑，runtime disable 时释放；source 与 ConversationIndex 的绑定只由 content composition root 持有并在 re-enable 时恢复。
  - ChatGPT Message toolbar 不再维护 mounted-surface inventory、MutationObserver 或 route watcher。它订阅共享 Materialization，但 pending typed anchor 只用于定位，不产生插件 DOM；只有唯一 `assistantMessageId` 已能由 Content Port `readTurn()` 读到权威 turn 且宿主已退出 streaming，才挂载到当前官方 action anchor 并直接显示词数。这样不会在首轮 canonical-route/React 提交窗口内改写宿主 action row。虚拟化卸载/重挂只改变 materialization，不改变 content token，也不会重复挂载。非 ChatGPT adapter 暂时保留旧 DOM-local toolbar lifecycle。
  ...
  - `ChatGPTDirectoryController` + `ChatGPTDirectoryRail` 是默认开启、用户可关闭的 ChatGPT right-side surface，由 `chatgptDirectory.enabled` 控制。目录宿主在 ChatGPT runtime 初始化时独立挂载到 `document.body`，使用与 lower-right controls 相同的 page-level fixed portal；body 尚未创建时才暂挂到 `document.documentElement`，后续 refresh 会迁移到 body，不等待语义内容 acquisition。有 V1 snapshot 时直接渲染 canonical rounds，source 尚不可用时仍保留宿主并等待后续 source/materialization 通知，避免目录因一次发现失败而消失。它使用 `ChatGPTConversationIndex` 连接当前 materialized anchors 与 typed identity；当前 DOM hydration window 变小不得缩短目录。rail 和 preview 的 right offset 由 viewport classic scrollbar 宽度加 `chatgptDirectory.rightInsetPx` 组成；用户边距默认 0px，只在滚动条覆盖目录条时由用户手动增加。仅当 `chatgptDirectory.enabled` 与 `chatgptDirectory.hideOfficialNavigation` 同时开启时，`ChatGPTOfficialNavigationVisibilityController` 才隐藏可确认的 ChatGPT 官方对话导航；隐藏链路以 ChatGPT conversation highlight root 的类名后缀 `_convSearchResultHighlightRoot` 作为容器锚点，只通过一条静态 CSS direct-child guard 隐藏该 root 下贴右侧的 fixed 直接子容器，不隐藏 root 本身，不读取布局，不改写官方 DOM，也不启动 observer 或 timer。选择器失配时必须 fail-open；左侧历史侧边栏与带 `data-aimd-role` 的自有节点必须明确排除。该能力不影响 Reader、Save Messages、复制或书签存储。
  ...
  - Reader、Save Messages 导出、当前消息 Copy Markdown / Copy PNG、书签保存与词数通过 `readerContentSource` 共享唯一 `ReaderItem[]` 正文投影。普通消费统一读取当前已发布 snapshot；`collectFreshReaderContent()` 保留为兼容名称，只委托无副作用的当前读取。显式 Reader Refresh 只等待或返回已观察到的本地工作，不能启动 baseline admission；`ConversationContentSourceV1` 不暴露 acquire/reconcile/coordinator API。路由/pageshow 只进入或重绑页面纪元并重扫 materialization；只有新纪元的首次被动内存读取或尚未关闭 gate 的真实官网 Graph capture 可以提交 baseline，任何信号都不能主动 GET 或重开 gate。ChatGPT provider dialect 在 discovery source Adapter 边界先规范化后再发布，Reader 投影不重复标准化；`conversationId + contentToken` 用于异步失效校验。ChatGPT 不使用消费者自有 DOM 正文 fallback，导出、复制、PNG 和书签不得各自选择正文发现或提取路径。
  - 当前消息 Copy Markdown 与 Copy PNG 可以在同一 content token 内共享一个 in-flight/resolved Reader item promise；该 identity 只用于异步事务校验，不进入书签、导出或存储 schema。Token 变化或 snapshot withdrawal 必须清空整页正文 promise；DOM mutation 只能使当前元素映射失效，不能创造正文。普通入口不主动 refresh；Route/dispose 同样清空，禁止跨 token 返回旧正文。
  - 官网 conversation Reader 由 `ChatGPTConversationReaderBinding` 订阅 Source state：精确内容前缀只追加尾项，既有 typed identity、prompt 或正文变化时原子替换并保留当前 identity，内容不变不更新；same-document stale 保留 last-good，只有 `snapshot=null`、route change 或 explicit unavailable 才关闭。Message toolbar 不再扫描 DOM turn tail、维护 known positions/branch cache 或触发消费者 flush。Detached Reader 继续采用不可变快照与手动 Refresh；Refresh snapshot 构造完成后必须再次校验 content token，annotation focus 在 Reader show 与 focus 两个异步提交点后也必须复核，迟到结果关闭旧 Reader 并 fail closed。
  - Save Messages 打开期间任何 ChatGPT source revision 变化都会关闭弹窗；书签保存弹窗返回后必须再次校验点击时的 source revision，不一致则拒绝写入。Copy/PNG 以点击时确认的 snapshot 为原子输入，不在执行中途切换正文。`source-backed`、`host-rendered` 与两者构成的完整 hybrid snapshot 可进入 whole-turn 输出；`stale` 全对话 Reader/导出与 reconstructed canonical output 必须 fail closed。
  ...
  - ChatGPT 工具栏书签高亮只被动读取 ConversationIndex；保存命令从当前 snapshot 的 Reader item/turn read 取得 typed identity、prompt、正文与绝对 position，再复用现有 `url + position` 书签身份。显式 element 无法精确映射时 fail closed，不改变底层存储 schema。
  - ChatGPT adapter 的 observer container 契约由唯一 `ChatGPTPageIndex` 持有；conversation root 替换、typed identity、完成 action anchor、Deep Research hydration 与当前 DOM anchor 都从这一个 Page Monitor 投影。`MessageToolbarOrchestrator` 在 ChatGPT 生产组合中不创建 `MutationObserver`、`RouteWatcher`、`ScanScheduler` 或 targeted recovery timer，只订阅 Materialization。pending typed surface/anchor 不允许注入插件节点；对应 assistant 已进入权威 Content snapshot、宿主已退出 streaming 且官方 anchor 仍 connected 后才挂载并显示数值词数。anchor 卸载时移除、稳定重挂时保持单例。`dirtyMessages + incremental reconcile` 仅为非 ChatGPT adapter 的现有兼容生命周期。
...
ChatGPT 内容发现与页面交互必须保持一个 canonical content SSOT、一个通用 Semantic Content Module、一个 surface join 和一个 navigation projection：
...
flowchart TD
    ...
    Repository["ConversationContentRepository<br/>V1 canonical snapshot"]
    Semantic["Semantic Content Module<br/>compile / resolve / project"]
    ReaderSource["readerContentSource"]
    ReaderItems["ReaderItem[]"]
    BodyConsumers["Whole-message Copy / Save Messages<br/>Bookmark / Export / Word count"]
    ...
    SurfaceProjection["Surface Projection<br/>canonical selection join"]
    ParserAdapter["Parser Capability Adapter<br/>formula/code source hints"]
    FormulaConsumers["Formula click / assets"]
    SelectionConsumers["Local selection Markdown<br/>future annotation anchors"]
    ...
    Materialization["ConversationMaterializationPortV1<br/>typed entries + bounded locate"]
    Alignment["UI visual alignment"]

    Graph --> Bridge
    Bridge --> Adapter
    Adapter --> Repository
    Repository --> ReaderSource
    ReaderSource --> ReaderItems
    ReaderItems --> Semantic
    Semantic --> StructureConsumers
    ReaderItems --> BodyConsumers
    ReaderItems --> RenderConsumers
    DOM --> PageIndex
    DOM --> SurfaceAdapter
    ...
    DOM --> ParserAdapter --> FormulaConsumers
    Repository --> ConversationIndex
...
- Reader、工具栏 Copy/Copy PNG、Save Messages 导出与书签保存正文必须只通过当前 snapshot 的 `readerContentSource` 获取正文，并消费同一份 `ReaderItem[]` 语义；Reader structure/outline 由 Semantic Content projection 提供。显式 Reader Refresh 只冲刷本地已观察工作；普通点击不得等待 refresh。页面局部选区必须优先走 `ContentSurfaceAdapter -> SurfaceProjection`，`host-rendered` 正文没有精确 provider source span 时必须 fail open，不能估算 offset 或退回 `Range.toString()`。
- ChatGPT 正文完整性、绝对顺序、branch identity 与 typed message identity 只由 `ConversationContentRepository` 的 V1 snapshot 负责；React props、内部 store 与 DOM Markdown 不得在消费者侧恢复成第二套 semantic fallback。Source Adapter 可提交一次完整 Graph 历史前缀；Host Monitor 只可提交经过稳定编译且连续锚定的 DOM 新尾部。已有长对话缺少基线时仍是 unavailable 并要求刷新，不能用当前虚拟化窗口冒充完整历史。
...
- Reader locate、书签 Go、跨页 pending navigation、lower-right stepper 与右侧目录条只消费冻结的 V1 Content Port；需要定位时再通过 `ChatGPTConversationIndex` / Materialization Port 连接 typed identity 与当前 connected anchors。Index 从 V1 source 获得 canonical order，并以 typed identity 连接 `ChatGPTPageIndex` 当前 connected anchors；Prompt 文本和 DOM-local position 永远不是 identity。
...
- 四个职责固定：V1 repository/source 回答“当前已验证内容是什么”；Semantic Content Module 回答“这份 Markdown 的稳定语义与 source span 是什么”；Surface Projection 回答“这次页面交互是否唯一对应当前 source revision”；ConversationIndex 回答“这一轮当前是否有锚点、如何 materialize 并定位”。PageIndex 只是 Index/Surface Adapter 的宿主结构输入，不是第三个内容源。
...
- `readerContentSource` 是 Reader 正文供给的共享 service 入口；Reader、Save Messages 导出、当前消息 Copy Markdown / Copy PNG 与书签保存正文均消费同一份 snapshot-backed `ReaderItem[]` 语义。普通入口直接读取当前投影；同一 immutable snapshot 与规范化页面 URL 命中 projection cache，不重新解析 Markdown；Reader header Refresh 只执行一次本地 flush 后重读当前投影，不能发起 Graph 获取。detached Reader 通过 `readerSession:refresh` 回源页执行同一 local-flush/read，并按 canonical typed identity 的唯一匹配保留当前页，仅对完全缺少 identity 的 legacy snapshot 使用唯一 position fallback。Reader Copy、Reader 选区源码复制、工具栏 Copy Markdown、消息书签 Markdown copy 与 ChatGPT 官网直选共同进入 `canonicalMarkdownCopy`；只在写入剪贴板前按 `formula.markdownCopyFormulaFormat` 重写公式 wrapper，默认 Markdown-dollar 不重复解析已经 canonical 的 Markdown。Reader 选区 action 与 Shadow DOM 原生 copy handler 不得各自决定公式格式。导出只将 `ReaderItem.content` resolve 为 `ChatTurn[]` 后交给既有 Markdown/PDF/PNG formatter，其中只有 Markdown formatter 会在写出文件前应用同一个 formula source format model；完整 source/hybrid/host-born 投影可导出，stale 或 reconstructed canonical output fail closed。
- ChatGPT 官网正文直选是一条严格限定的 selection exit，不是新的消息正文来源：只要 ChatGPT 平台 runtime 开启，唯一的 `ChatGPTAtomicSelectionController` 就保持一个 document `selectionchange` listener、一个 window `keydown/keyup` listener 和一个 window bubbling `copy` listener；`chatgptBehavior.atomicMarkdownCopyShortcut` 只选择 `none`、`mod-c` 或 `mod-shift-c`，不启停原子选区能力。controller 只在同一条非流式 assistant `.markdown.prose` 内读取当前单 Range，并用 `requestAnimationFrame` 合并选区变化；任何并列 atom 仍为部分选中时整体 fail-open，但完整内层 atom 不受部分结构祖先影响。原子识别独立驱动完整单元高亮；Markdown 序列化失败只会取消复制接管，不得反向取消已确认的原子状态。合法选区只建立一个短生命周期 `PageAtomicSelectionSnapshot`：selection-aware clone 只遍历 Range 相交节点并复制必要祖先；完整 KaTeX 容器在遍历入口立即通过共享 `extractLatexSource` 变成只含 authoritative TeX 的轻量原子，因此公式视觉树、MathML 辅助层和未选中的共同祖先内容都不会进入解析预算。紧凑 fragment 完整进入共享 `copyMarkdownFromElement` 服务，统一执行 `adapter.normalizeDOM`、宿主噪声过滤、浏览器折叠空白归一化、DOM→Markdown parser 与 `adapter.cleanMarkdown`，得到唯一 canonical Markdown；公式 wrapper 只在实际 Markdown 剪贴板出口经共享 `canonicalMarkdownCopy` finalizer 应用。`mod-c` 只在真实 `keydown → copy` 意图下清空宿主 clipboard types 并写入 canonical `text/plain`；`mod-shift-c` 在合法选区用户手势中直接写入；`none` 完全交还宿主。该入口不再创建选区复制按钮或反向 action，不生成富 HTML、MathML、Office Math 或 Word payload，也不加载 Reader/export renderer；单公式 MathML 资产动作仍是独立能力。输入控件、DevTools 抢占、无完整原子、跨消息、流式消息、不可逆源码、紧凑 fragment 超预算或 canonical snapshot 失败时 fail-open。链路不建立 staged DOM、不替换原生选区、不停止第三方事件传播，也不新增第二套 controller、MutationObserver、逐消息 listener、异步选区任务或持久缓存；该链路不能替代 Reader、toolbar、bookmark、Save Messages 或 export 的 fresh `ReaderItem[]` 主链。
