...
- Reader 当前已经拥有两条稳定的“只在 Reader 内部生效”的扩展链路：
  ...
  - inline comments：comment session、highlight overlay、右侧 gutter anchor 与 Reader 内注释列表/删除仍局限在 Reader overlay 内，不依赖 background/storage；但 comment export 的 prompt/template/prompt-position/sort-mode 配置已提升到 settings 域持久化，Reader 注释列表、Reader export popover 和 Reader Send 插入注释共用同一排序语义，export popover 只负责预览与复制最终结果
