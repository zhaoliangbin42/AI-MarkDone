...
- ChatGPT 当前的专属增强能力采用 **Conversation Content Port V1、fail-closed** 内容发现。2026-08-04 起，生产 content runtime 由 `ConversationContentRepository` + `ChatGPTConversationDiscoveryAdapter` + 唯一 `reconcile()` 协调；旧 Engine/Reducer 已从生产代码和测试入口删除。受限 active GET 因真实浏览器门禁尚未通过而保持关闭；被动 graph 与 verified typed DOM partial 是当前可用来源。
  ...
  - page bridge 通过 manifest `document_start` 在页面主执行环境安装，只被动旁路观察 ChatGPT 页面自身的 same-origin `GET /backend-api/conversation/<id>` JSON graph；它不观察 generation POST/SSE、请求体或生成响应。AI-MarkDone 不主动请求 conversation/session，不读取 Cookie、Token 或认证请求头，也不构造认证信息。capture 只携带 conversation ID 与单调 sequence；repository 每次只读取一次 bridge 内存。同一 epoch 的显式 reconcile 共用 single-flight，route epoch 不匹配的 graph、DOM facts 与 Promise 结果直接丢弃，不再维护退避重试、5 秒轮询或 visibility rebuild。
  - `ChatGPTDomTurnFactSource` 复用唯一 `ChatGPTPageIndex` observer 与既有 Markdown copy service，同时报告 typed identity、streaming/completed 状态和规范正文。空白 route 为见证 birth 立即激活；晚接入 `/c/:id` 在取得 graph 证明前不启动 DOM facts，避免扫描一段不能证明完整历史的 DOM，graph 建立后才继续完成 pending tail 与观察连续 successor。Snapshot 只发布 position 1 起连续、身份唯一、assistant 正文非空的稳定完成前缀；首轮 streaming 时保持 `collecting + null`，官方完成操作栏挂载、停止 streaming 且多 assistant segment 合并后的 Markdown 非空后才一次发布。后续生成期间保留旧稳定前缀，新轮完成后只增加一次 revision。Observer 只监听必要 `childList` 与 identity attributes，不监听 `characterData`，一批 mutation 只归约一次。
  - graph 决定完整分支、绝对顺序与 typed identity；birth 只能从零开始并沿连续 successor 推进。同 lineage 的旧 pending/空 graph 不得回滚已完成正文，后到非空 graph 重新取得正文权威，graph 与 birth 冲突时 graph 原子替换。当前 materialized typed identity 与已发布 round 冲突时立即 `blocked: identity-conflict` 并撤下旧 snapshot，等待完整 graph 重新校准，避免 Reader、导出或书签继续消费旧分支。
  - Chrome/Chromium 继续使用 object `CustomEvent.detail`，Firefox 使用 JSON string detail 规避 content/page script 隔离边界。该差异只能存在于 bridge transport encode/decode 层；Reader、Bookmark、Copy、Save Messages 与 ChatGPT 消息词数通过 `ConversationContentSourceV1` / `readerContentSource` 消费同一个 V1 snapshot，不得新增浏览器分支或消费者自有 DOM fallback。
  ...
  - Reader、Save Messages 导出、当前消息 Copy Markdown / Copy PNG、书签保存与词数通过 `readerContentSource` 共享唯一 `ReaderItem[]` 正文投影。`readCurrentReaderContent()` 只读已发布 snapshot 且没有副作用，供词数、bookmark active 与 Reader revision binding 使用；`collectFreshReaderContent()` 只服务真实用户动作，内部最多调用一次无参数 `ensureReady()`，调用方不能选择发现策略。规范化正文只按 immutable snapshot identity 复用，`routeEpoch + revision + conversationId` 只用于 stale 校验；ChatGPT 不使用 DOM 正文 fallback，导出、复制、PNG 和书签不得各自选择正文发现或提取路径。
  - 当前消息 Copy Markdown 与 Copy PNG 可以在同一 source revision 内共享一个 in-flight/resolved fresh Reader item promise；revision identity 固定为 `routeEpoch + revision + conversationId`，只用于异步事务校验，不进入书签、导出或存储 schema。Source revision 或 snapshot withdrawal 必须清空整页正文 promise；DOM mutation 只能使当前元素映射失效，不能创造正文。Route/dispose 同样清空，禁止跨 revision 返回旧正文。
  - 官网 conversation Reader 由 `ChatGPTConversationReaderBinding` 只订阅 Source revision：精确内容前缀只追加尾项，既有 typed identity、prompt 或正文变化时原子替换并保留当前 identity，内容不变不更新；`snapshot=null`、route change 或 identity conflict 立即关闭。Message toolbar 不再扫描 DOM turn tail、维护 known positions/branch cache 或触发消费者 flush。Detached Reader 继续采用不可变快照与手动 Refresh；Refresh snapshot 构造完成后必须再次校验 Source revision，annotation focus 在 Reader show 与 focus 两个异步提交点后也必须复核，迟到结果关闭旧 Reader 并 fail closed。
