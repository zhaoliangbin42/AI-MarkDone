...
- `content.js` 仍是 manifest 直接声明的 classic content script，只包含站点启动、controller、port 与 lazy loader；`ReaderPanel`、`BookmarksPanel`、`SaveMessagesDialog`、`BookmarkSaveDialog`、Copy PNG 和 direct-rich Markdown renderer 实现不得被它静态导入。
...
- `content-features.js` 是保留公开导出的 ES module facade；每个 facade 方法再按 Reader、Bookmarks、Save Messages、bookmark save、Copy PNG 或 direct-rich copy 的实际触发分别加载对应 chunk。它与 detached `reader.js` 在同一个 Rollup graph 中构建，以共享 Markdown / Reader 依赖而不复制整套 renderer。
...
- ChatGPT 当前的专属增强能力采用 **dual-proof reducer、fail-closed** 内容发现：
  - `ChatGPTConversationReducer` 是唯一 semantic SSOT，`ChatGPTConversationEngine` 只协调当前 route epoch、保存状态并提供 `getState / subscribe / ensureReady`。Canonical snapshot 只接受 `observed-graph` 与 `birth-epoch` 两种完整性根证明：前者来自 page bridge 被动观察到的宿主完整当前分支 graph；后者只在扩展亲历无 conversation ID、零 typed turn 的空白 epoch，并连续见证首轮 streaming 到官方完成后成立。晚接入但没有 birth/graph 时严格 `blocked: unproven-history`，不得用可见轮次、prompt 文本或 DOM-local position 猜测完整性。
  - page bridge 通过 manifest `document_start` 在页面主执行环境安装，只被动旁路观察 ChatGPT 页面自身的 same-origin `GET /backend-api/conversation/<id>` JSON graph；它不观察 generation POST/SSE、请求体或生成响应。AI-MarkDone 不主动请求 conversation/session，不读取 Cookie、Token 或认证请求头，也不构造认证信息。capture 只携带 conversation ID 与单调 sequence；Engine 每次只读取一次 bridge 内存。同一 epoch 的显式 flush 共用 single-flight，route epoch 不匹配的 graph、DOM facts 与 Promise 结果直接丢弃，不再维护退避重试、5 秒轮询或 visibility rebuild。
  ...
  - 官网 conversation Reader 由 `ChatGPTConversationReaderBinding` 只订阅 Source revision：精确内容前缀只追加尾项，既有 typed identity、prompt 或正文变化时原子替换并保留当前 identity，内容不变不更新；`snapshot=null`、route change 或 identity conflict 立即关闭。Message toolbar 不再扫描 DOM turn tail、维护 known positions/branch cache 或触发消费者 flush。Detached Reader 继续采用不可变快照与手动 Refresh；Refresh snapshot 构造完成后必须再次校验 Source revision，迟到结果 fail closed。
...
- ChatGPT 官网正文直选是一条严格限定的 selection exit，不是新的消息正文来源：`ChatGPTAtomicSelectionController` 由默认开启的 `chatgptBehavior.atomicMarkdownCopy` 增强选区总开关控制，Settings 关闭时立即 dispose，重新开启时立即 init，无需刷新。它只在同一条非流式 assistant `.markdown.prose` 内读取当前单 Range，用一个 document `selectionchange` listener + `requestAnimationFrame` 识别完整 rendered atomic units；任何并列 atom 仍为部分选中时整体 fail-open，但完整内层 atom 不受部分结构祖先影响。合法选区只建立一个短生命周期 `PageAtomicSelectionSnapshot`：selection-aware clone 只遍历 Range 相交节点并复制必要祖先；完整 KaTeX 容器在遍历入口立即通过共享 `extractLatexSource` 变成只含 authoritative TeX 的轻量原子，因此公式视觉树、MathML 辅助层和未选中的共同祖先内容都不会进入解析预算。紧凑 fragment 进入 Reader、toolbar 与 bookmark 共用的 `copyMarkdownFromElement` 服务，统一执行 `adapter.normalizeDOM`、宿主噪声过滤、浏览器折叠空白归一化、DOM→Markdown parser 与 `adapter.cleanMarkdown`，得到唯一 canonical Markdown，避免粗体、列表、代码或公式在边缘处失去闭合。parser 的 abort 必须穿透局部 error boundary，并在 cleaner 之前转为失败；不得再退化为拼接 `textContent`。缺少可逆公式源码时返回 `null`，不得把视觉文本或 `outerHTML` 冒充 TeX。snapshot 只保留宿主 Range/root/atomic units、canonical Markdown 与最终 plain Markdown；fragment 在构建结束后即可释放。`Command/Ctrl+C` 由唯一常驻 window bubbling `copy` listener 持有，清理已有 clipboard types 后只写 `text/plain`，在 ChatGPT document-level handler 完成后取得最终所有权但不停止传播。合法 snapshot 旁只显示一个“带格式复制”action；选区变化只建立 snapshot 和 action，不构建富 HTML、不加载 MathJax，也不存在公式准备态。点击 action 后才把 canonical Markdown 交给 Reader 同源的 GFM/math parser 与 sanitizer，输出 basic semantic `text/html` 和 snapshot 的最终 Markdown `text/plain` representation；rich implementation 不再读取 page DOM、Range、SiteAdapter 或公式元素，不拥有第二套提取/清洗 traversal。公式按独立的 `formula.richCopyFormulaFormat` 从 canonical math node 生成五种 source wrapper 之一，默认 Markdown dollar，并与单公式 `clickCopyFormulaFormat`、全部 Markdown 出口 `markdownCopyFormulaFormat` 分离。rich HTML 不复制页面 MathML、raw HTML、宿主 CSS/class、chrome 或扩展高亮；写入优先 `ClipboardItem`，失败后可尝试一次性 bubbling native copy event，最终失败明确报错，不能把 text-only 写入冒充成功。Word Office Math 与结构化 MathML 已从 direct-rich 产品入口和 runtime 链路移除，旧设置值归一为 Markdown dollar；单公式 MathML 资产动作仍独立保留。该链路不建立 staged DOM、不替换原生选区、不停止传播，也不新增常驻 listener、第二套 controller、MutationObserver、逐消息 listener、异步选区任务或持久缓存。无完整原子、跨消息、流式消息、不可逆源码、紧凑 fragment 超预算或 canonical snapshot 失败时保持官网原生复制；该链路不能替代 Reader、toolbar、bookmark、Save Messages 或 export 的 fresh `ReaderItem[]` 主链。
