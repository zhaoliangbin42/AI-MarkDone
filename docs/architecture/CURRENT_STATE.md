...
- ChatGPT 当前的专属增强能力采用 **canonical graph-first、fail-closed** 内容发现：
  - `ChatGPTConversationEngine` 是唯一 semantic SSOT。page bridge 通过 manifest `document_start` 在页面主执行环境安装，只被动旁路观察 ChatGPT 页面自身的 same-origin `GET /backend-api/conversation/<id>` JSON graph；它不观察生成用 POST、请求体或 event stream。AI-MarkDone 不主动请求 conversation/session，不读取 Cookie、Token、认证请求头、请求体或生成响应，也不构造认证信息。bridge 只在内存中有界保留 graph；content world 再校验 conversation ID、source、coverage、branch identity、连续位置与 typed message identity 后才发布 Canonical conversation snapshot。实时新回复复用 `ChatGPTPageIndex` 的 host mutation 信号：仅当当前 canonical 尾轮次仍唯一映射到 DOM，且其后出现 typed identity 完整、官方完成操作栏已挂载、正文非空的连续 successor 时，`ChatGPTLiveDomContent` 才用既有 Markdown copy service 生成 live tail 并写回同一 Engine snapshot。未知前驱、未完成或身份冲突均 fail closed；React props、内部 bundle/store 与 DOM Markdown 均不是历史完整性 fallback，首次 graph 不可用时 fail closed。
  - graph 基线允许最后一轮暂时只有 user identity 或空 assistant 正文；这不是完成态。仅当该 canonical 尾轮仍由唯一 typed identity 对应到已挂载 DOM、assistant 官方完成操作栏存在且复制出的 Markdown 非空时，`ChatGPTLiveDomContent` 才可原位完成同一 position，随后再发布连续 ready successor。Reader/word-count 的显式 force refresh 必须在返回前同步冲刷这一待处理 reconciliation，而不是把 `force` 解释为主动发起认证请求。已由 DOM 完成证明推进的同身份正文采用单调合并：后到的旧空 graph 不得回滚；更新 graph 提供非空正文时重新取得正文权威；assistant identity 冲突表示真实分支变化并以 graph 为准。后续未完成轮只能终止本次连续前缀，不能撤销此前已验证完成的前缀。
