...
- 2026-08-26 全量消息获取规则：跨消息内容只在显式空 `?message=` 刷新后执行一次有界 DOM materialization sweep；该任务复用同一 PageIndex、Host Monitor、Conversation Surface 与 Repository，可取消、可重试，不引入第二 observer、网络 acquisition path、轮询器或逐消息 timer。旧的显式到顶动作已删除，不再属于生产能力。
