...
3. Extension page 复用 ReaderPanel、Reader settings surface、Markdown rendering、copy/comment/Sticky/prompt 与 conversation Reader action service；发送弹框必须复用同一个 tokenized SendPopover，通过 SendPort 在 content adapter 与 detached `readerSession:send` 之间切换，不得退回 `window.prompt` 或一次性原生弹框
