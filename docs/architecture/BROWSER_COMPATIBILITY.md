...
- ChatGPT direct rich selection copy 不做 Chrome/Firefox 或 Word 版本分支。当前只从已经清洗的 canonical Markdown 经 Reader 同源 parser/sanitizer 输出基础 semantic HTML 与五种可配置的公式源码 wrapper，不读取 page DOM 或页面 MathML；该 renderer 只在点击“带格式复制”后经现有 content-feature facade 懒加载，异步期间选区变化会丢弃旧结果。优先用 `ClipboardItem` 同时写入 `text/html` + 最终 Markdown `text/plain`，不可用时再尝试一次性原生 `copy` event。选区变化和键盘 Markdown copy 不生成富 payload、不加载 Reader renderer 或 export renderer；Word Office Math 与结构化 MathML 暂不属于该入口。任一富写入最终失败都必须明确报告，不能替换原生选区或静默把 text-only 写入冒充成功。
