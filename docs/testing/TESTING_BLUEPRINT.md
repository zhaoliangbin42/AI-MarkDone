...
- Repository scenario tests cover one baseline gate per conversation epoch, epoch cancellation, single-flight passive admission, baseline-prefix/host-tail merge, host-born projection, sealed-turn direct reads, virtualized-window regression, suffix regeneration, historical-prefix stale behavior, and explicit unavailable states. Ledger property scenarios remain a reducer seam for arbitrary event order, duplicate idempotency, first-write sealing, conflict visibility, and gap closure; the Ledger is not a second production repository.
- Adapter/bridge tests cover passive memory, malformed/partial payloads, strict route identity, zero extension conversation requests, closed-gate capture no-ops, and Chrome object versus Firefox JSON transport parity.
...
- The existing-conversation fixture admits one Graph baseline, appends multiple stable DOM tail turns, ignores later Graph captures, and proves Reader, copy, bookmark, export and word-count consumers remain aligned. A partial baseline with one streaming tail can be closed by the corresponding stable DOM turn.
