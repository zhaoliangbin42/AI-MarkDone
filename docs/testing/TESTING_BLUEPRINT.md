...
- Repository scenario tests cover one baseline gate per conversation epoch, epoch cancellation, single-flight passive admission, append-only cache growth, host-born projection, direct reads for acquired turns, virtualized-window regression, duplicate assistant identity idempotency, compiler retry and explicit unavailable states.
- Adapter/bridge tests cover passive memory, malformed payloads, generating-tail omission, strict route identity, zero extension conversation requests, closed-gate capture no-ops, and Chrome object versus Firefox JSON transport parity.
- The first-turn lifecycle fixture follows `/` → temporary `/c/WEB:*` → canonical `/c/:id`, proves typed DOM facts survive canonical identity binding, and enters through shared Materialization. A pending typed anchor must leave the official action row untouched; only the `host-born/complete` snapshot may mount one toolbar with numeric word count, while the official action and completed stop state remain stable. No domain/path empty-page inference is allowed.
- Route identity tests must keep `/c/<id>`, `/conversation/<id>`, and nested `/g/<scope>/c/<id>` coverage distinct from negative `/g/<id>`, `/share/<id>`, query-only `conversationId`, and URL-stable anonymous cases. The route parser is a shared identity rule, not a general URL or empty-page detector.
- The existing-conversation fixture admits one Graph baseline, appends multiple stable DOM turns, ignores later Graph captures, and proves Reader, copy, bookmark, export and word-count consumers remain aligned. A generating tail is absent from the cache until the corresponding stable DOM turn is compiled.
- Anonymous stable-URL discovery is a pending gate, not current coverage. Before it is enabled, tests must prove page-session identity isolation, DOM seed without a Graph baseline, same-URL New Chat reset, virtualized unmount/remount preservation, late attach limits, and exclusion from persistent bookmark/cross-page navigation contracts.
- Page Monitor coverage proves 1,000 streaming character mutations produce zero pre-stable compile and at most one post-stable compile, while unrelated host churn/localized `aria-label`, extension-owned nodes and virtualized remounts do not create content work. Deep Research iframe hydration must travel through the shared lifecycle signal.
- Coordinator coverage proves only a real bridge capture may retry an already-attempted open baseline gate; epoch entry performs the one initial passive-memory read. No finite retry window, consumer refresh, polling loop or DOM mutation may replay Bridge admission after the gate closes. Governance coverage rejects public coordinator/acquire/reconcile methods from `ConversationContentSourceV1`.
...
- Materialization and consumer-boundary tests prove DOM remounts do not change semantic tokens, an assistant-only virtualized remount rebinds to the cached assistant without appending content or duplicating a toolbar, Directory active geometry remains stable, ChatGPT toolbar owns no second observer/route watcher, pending anchors do not inject into React-owned action rows, and consumers do not add their own DOM fallback discovery.
