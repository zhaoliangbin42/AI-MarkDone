...
- Repository scenario tests cover no-document `idle` without a route failure, page-scoped DOM publication, identity promotion without content-token churn, one baseline gate per canonical epoch, canonical switch cancellation, same-URL id-less reset, DOM-first atomic batches, Graph-first admission, verified late historical-prefix insertion, direct reads for obtained turns, virtualized-window regression, duplicate assistant identity idempotency, compiler retry and baseline-failure unavailable states.
- Adapter/bridge tests cover passive memory, arbitrary website-owned GET paths/query locations, malformed payloads, generating-tail omission, exact route identity, bounded Graph traversal, zero extension conversation requests, no POST observation, closed-gate capture no-ops, and Chrome object versus Firefox JSON transport parity.
...
- The existing-conversation fixture covers both orders: one Graph baseline followed by multiple stable DOM turns, and a stable DOM batch followed by a Graph that adds only a verified historical prefix. Reader, copy, formula, export and word-count consumers remain aligned; bookmark joins only with canonical identity, and a generating tail is absent until the corresponding stable DOM turn is compiled.
...
- Runtime composition coverage proves canonical epoch entry performs one passive-memory read and only a real Graph capture may wake an open gate. `ChatGPTConversationContentRuntime` owns PageIndex, page/canonical identity, History `pushState`/`replaceState`, `popstate`, `hashchange`, `pageshow`, and Graph-before-identity synchronization; A→B immediately fences old staging/compiler work. No finite retry window, consumer refresh, polling loop or ordinary DOM mutation may replay Bridge admission after the gate closes. Governance rejects a discovery Coordinator, content RouteWatcher, or public acquire/reconcile methods on `ConversationContentSourceV1`.
