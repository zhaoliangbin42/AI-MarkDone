...
- Repository scenario tests cover page-scoped DOM publication, page-to-canonical identity promotion, per-conversation tab-local pool switching, direct reads, current-DOM ordering, virtualized-window preservation, duplicate assistant identity idempotency, and latest eligible DOM body updates.
...
- The first-turn lifecycle fixture observes assistant DOM before canonical identity and proves it publishes only after the official action row exists and generation has ended. Toolbar word count, Directory, Stepper, Reader, copy and export use that pool; formula actions use formula DOM directly; bookmark is disabled and sends no save/remove request. A later `/c/:id` promotion preserves projection/content tokens and enables the existing bookmark chain.
...
- The existing-conversation fixture loads additional rendered windows over time. Every message with an official completion anchor enters the active pool in current DOM order; virtualized removal does not shrink it. Reader, copy, export and word-count consumers remain aligned, while a generating message stays absent until completion.
...
- Page Monitor and Host Monitor coverage proves initialization, delayed official action-row hydration, generation end, root replacement and relevant content mutation all reach one short page-level reconciliation. Streaming content is not compiled; equal bodies are no-ops; changed eligible DOM updates the same identity; extension-owned and unrelated mutations are ignored. Deep Research iframe hydration continues through its verified Adapter anchor.
