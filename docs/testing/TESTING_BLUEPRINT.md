...
- Repository scenario tests cover no-document `idle` without a route failure, page-scoped DOM publication, identity promotion without content-token churn, one baseline gate per canonical epoch, canonical switch cancellation, same-URL id-less reset, DOM-first atomic batches, Graph-first admission, full-envelope monotonic Graph merge, closed-gate Graph upgrade, strict identity/order rejection, direct reads for obtained turns, virtualized-window regression, duplicate assistant identity idempotency, compiler retry and baseline-failure unavailable states.
- Adapter/bridge tests cover passive memory, arbitrary website-owned GET paths/query locations, malformed payloads, generating-tail omission, exact route identity, bounded Graph traversal, zero extension conversation requests, no POST observation, bridge memory replay through the existing bounded upgrade seam, and Chrome object versus Firefox JSON transport parity.
- The first-turn lifecycle fixture observes typed DOM facts before canonical identity and proves one stable round immediately publishes as `host/complete`. Toolbar word count, Directory, Stepper, Reader, copy, formula and export use that pool; bookmark is disabled and sends no save/remove request. A pending typed anchor leaves the official action row untouched. A later `/c/:id` promotion preserves projection/content tokens, enables the existing bookmark chain, and keeps official actions plus completed send state stable.
...
- The existing-conversation fixture covers both orders: one Graph baseline followed by multiple stable DOM turns, and a stable DOM batch followed by a Graph whose complete envelope contains the maintained turns. A later Graph may add hidden prefix, middle, or tail turns only when typed identity/order containment is proven. Reader, copy, formula, export and word-count consumers remain aligned; bookmark joins only with canonical identity, and a generating tail is absent until the corresponding stable DOM turn is compiled.
- URL-stable anonymous discovery is current page-local behavior. Tests prove page-session isolation, same-URL New Chat reset only after clear plus a later first-round generation start, atomic replacement only after compilation, virtualized unmount/remount preservation, late-attach limits, and exclusion from persistent bookmark/cross-page navigation contracts.
- Page Monitor and Host Monitor coverage proves persistent slots may hydrate across separate mutation batches; generation start/end and same-owner assistant identity replacement survive those batches; 1,000 streaming character mutations produce zero pre-stable compile and at most one post-stable compile; action anchor, generation end, or a later typed round is a strong completion signal; an otherwise complete turn uses one bounded 2-second quiet confirmation; unique mounted pool-tail order wins over generation evidence; unresolved gaps, root replacement, stale compilation, unrelated host churn, localized `aria-label`, extension-owned nodes and virtualized remounts cannot corrupt content. Deep Research iframe hydration must travel through the shared lifecycle signal.
- Runtime composition coverage proves canonical epoch entry performs one passive-memory read and a real Graph capture may wake either the open baseline gate or the bounded upgrade seam. `ChatGPTConversationContentRuntime` owns PageIndex, page/canonical identity, History `pushState`/`replaceState`, `popstate`, `hashchange`, `pageshow`, document `resume`, and Graph-before-identity synchronization; `pageshow` and `resume` share one 50 ms wake reconciliation, and A→B immediately fences old staging/compiler work. No finite retry window, consumer refresh, polling loop or ordinary DOM mutation may create a new acquisition path after the gate closes; only a real capture or explicitly armed lifecycle/Settings probe may compare bridge memory. Governance rejects a discovery Coordinator, content RouteWatcher, or public acquire/reconcile methods on `ConversationContentSourceV1`.
- Bridge coverage includes a graph whose unfinished final round is omitted, and asserts the surviving provider positions are renumbered before Content Port validation.
- Surface and consumer-boundary tests prove one Frame atomically joins obtained turns with pending/mounted/unmounted anchors; DOM remounts do not change semantic tokens; an assistant-only virtualized remount rebinds without appending content or duplicating a toolbar; Directory visibility/items/geometry reconcile together; ChatGPT Directory/Toolbar/Stepper/navigation own no second join, observer or route watcher; pending anchors do not inject into React-owned action rows; and consumers do not add their own DOM fallback discovery. Governance also requires the retired Discovery Coordinator, Conversation Index, standalone Materialization implementation, and source-only V2 integration harness/tests to remain absent.
...
- Source/Host Adapter tests lock provider-dialect normalization, typed per-turn evidence, observe-then-snapshot behavior, stable compiler capture, source/host independence, and the rule that a later observation cannot overwrite an acquired cache entry. The generic compiler corpus perturbs wrappers and covers paragraph, list, table, code, quote, link, image, and formula semantics with fail-closed unsupported input.
...
- Surface Projection tests are the only source/surface join tests. They cover current content/materialization validation, validated `host-rendered` acceptance, reconstructed rejection, and canonical fragment recovery. Real trigger tests separately prove a re-captured Range and surface token invalidate remounted interaction state.
