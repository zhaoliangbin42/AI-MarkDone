...
For ChatGPT content-discovery or conversation-root replacement changes, the real trigger path must prove an empty new-conversation epoch reaches one ready first-round snapshot after streaming completes even when no graph capture occurs; a graph baseline still supplies every virtualized round; subsequent typed DOM successors append once; and regenerate identity conflict withdraws the old snapshot before export. Coverage must also prove PageIndex owns the single shared mutation signal with no `characterData` observation, ConversationIndex reads the bound Source directly and only projects current state to anchors, a unique mounted assistant `data-message-id` resolves to the canonical `assistantMessageId` even when host wrapper/turn IDs drift, ambiguous identities fail closed, route-epoch-late facts are ignored, concurrent `ensureReady` calls share one flush, and runtime disable→enable rebuilds and rebinds the projection without duplicate subscriptions. Passive word-count, bookmark-active, Index, and Reader revision updates must not call `ensureReady`; each user command may confirm at most once through `readerContentSource`. Reader exact-prefix extension appends once, canonical correction replaces atomically, and snapshot withdrawal closes Reader and Save Messages; bookmark writes returning from a dialog and Detached Reader refresh snapshot construction must reject a changed source revision. Focused coverage must include ConversationReducer, Engine birth and bridge transport, ConversationIndex, PageIndex, DOM turn facts, directory controller, Reader binding, Reader/export/bookmark/toolbar entrypoints, content entry, and toolbar scheduler tests. Static boundary coverage must reject active conversation/session requests, generation POST/SSE observation, authentication data access, consumer-side retry polling, a second content-discovery observer, consumer `setSnapshot`, UI/controller `ensureReady`, UI/controller `buildChatGPTReaderItems`, duplicate Reader content entrypoints, and ChatGPT DOM-tail discovery.
