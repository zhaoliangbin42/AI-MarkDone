...
For ChatGPT conversation-root replacement changes, the real mutation trigger path must prove the directory keeps the complete canonical count while anchors are replaced, an unknown typed anchor forces semantic refresh, and the toolbar mounts exactly once in the replacement root. Coverage must also prove PageIndex is structural only, ConversationIndex joins anchors by unique typed identity, streamed content does not create a second semantic source, and runtime disable→enable rebuilds and rebinds the projection. Focused coverage must include Engine, ConversationIndex, PageIndex, directory controller, message stepper, content entry, and toolbar scheduler tests.
...
For ChatGPT content-discovery changes, browser evidence is part of the gate, not an optional manual check. The only accepted semantic source is a verified same-conversation graph passively observed from ChatGPT's own same-origin `GET` response; the extension must not request a session/conversation, read Cookie/Token/authentication headers, or construct authentication information. React props, internal stores and visible DOM are not completeness fallbacks. Invalid, incomplete, cyclic, stale-route, or malformed graph data must fail closed or preserve only the permitted verified same-conversation snapshot. PageIndex may expose connected structural anchors but never content/order truth; ConversationIndex must preserve the complete Engine count while virtual windows change and materialize off-screen targets by exact typed identity. Focused coverage must include static no-auth/active-fetch guards plus `ChatGPTConversationEngine.test.ts`, `chatgptConversationBridge.test.ts`, `ChatGPTConversationIndex.test.ts`, `ChatGPTPageIndex.test.ts`, `readerContentSource.test.ts`, directory/stepper/navigation caller tests, toolbar/Reader mapping, lifecycle disable→enable, and a live long-conversation route/scroll/navigation check.
