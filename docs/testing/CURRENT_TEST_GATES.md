...
### ChatGPT directory current override (2026-08-07)

The active directory gate follows [ADR-0015](../adr/ADR-0015-chatgpt-passive-graph-directory.md):
the document-start bridge only observes the website-owned conversation `GET`,
`peek()` never performs network I/O, and the directory must read graph prompts
from the bound `ConversationContentRepository`/`ChatGPTConversationIndex`.
The test must reject active conversation acquisition, `Message N` DOM fallback,
ratio/pixel probing, and synthetic discovery scrolling. A missed first passive
response is resolved only by refreshing the page.
...
For ChatGPT content-discovery or conversation-root replacement changes, the real trigger path must prove `/` → temporary `/c/WEB:*` → canonical `/c/:id` publishes typed turns only from the Source Graph through one bounded same-origin read when passive replay is absent. If that read is unavailable, DOM body/position evidence must never be promoted; `readTurn()` may serve an already sealed source message. A graph baseline still supplies every virtualized round; a graph with an unfinished final assistant is unavailable for that tail, while an internal empty shell followed by a continued branch must not hide later complete turns; an all-pending graph is unavailable, never an empty ready snapshot. Source evidence is never merged with a DOM-shaped candidate; proof is judged by `order/bodies/tail/gaps`, and virtualized shrink only removes materialization anchors. Coverage must prove epoch/revision fencing, branch replacement without suffix mixing, first-write sealing, duplicate idempotency, explicit identity conflict, resource completion binding, and no request bodies, generation payloads, cookies, tokens, auth/session headers, or cloned SSE responses. The single PageIndex observer must ignore unrelated host child-list churn/localized `aria-label` changes while forwarding typed lifecycle changes. ConversationIndex reads the bound Content Port directly and only projects current state to anchors; ambiguous identities and late route results fail closed; concurrent `refresh()` calls share one acquisition; and runtime disable→enable rebuilds the projection without duplicate subscriptions. Real route/pageshow/generation/host/refresh signals may trigger source acquisition; there is no timer-based content recovery or polling. Reader and local message actions read the shared source/turn port; complete export waits for proof closure, and delayed bookmark, Detached Reader, and annotation actions reject a changed content token. Provider Markdown is normalized once at the Source Adapter edge, while rendered content uses the generic compiler and injected parser capabilities; a later host observation cannot overwrite a sealed digest. Focused coverage must include `ConversationEvidenceLedger`, `StableTurnCapture`, `RenderedContentCompiler`, `ConversationContentRepository`, bridge transport, `ConversationIndex`, `PageIndex`, DOM turn facts, discovery adapter/coordinator, materialization, directory controller, Reader binding, Reader/export/bookmark/toolbar entrypoints, content entry, and toolbar scheduler tests. Static boundary coverage must reject conversation/session credential reads, generation body/SSE parsing, endpoint cascades, consumer-side retry polling, a second content-discovery observer, consumer `setSnapshot`, UI/controller `ensureReady`, UI/controller `buildChatGPTReaderItems`, duplicate Reader content entrypoints, and ChatGPT DOM-tail discovery.
...
For the delayed ChatGPT discovery branch, the selected gate must also include
the missed-capture replay scenario: the document-start bridge must replay its
latest graph to a late runtime subscriber; if replay is unavailable, only a
real route/pageshow/generation/host/refresh signal may request another source
read. There is no timer-based recovery or content polling. A graph containing
an internal empty shell must still publish later complete source turns with
continuous canonical ordinals, while an unfinished final tail remains
unavailable until a later source graph proves completion.
# Current ChatGPT discovery gate (V2)

The active ChatGPT gate is the V2 sparse contract, not the historical Source Graph/Page Bridge contract described in older sections of this document. The required focused evidence is:

- `SlotTopologyDecoderV2`: complete shell count/order, nested marker removal, sentinel/role-phase proof, prepend/append, duplicate/ambiguous topology fail-closed.
- `ChatGPTVirtualConversationHostAdapter`: observe-before-snapshot, one Observer, existing-shell hydration, root replacement, mount/unmount, streaming lifecycle, no network and no synthetic scroll.
- `RenderedContentCompilerV2`: generic semantic HTML, formula/code source carriers, wrapper/noise tolerance, semantic-signature rejection, budgets, and no glyph-text Markdown/TeX fallback.
- `ConversationDiscoveryModuleV2`: epoch/batch fencing, immutable first seal, sparse `ready/unavailable`, independent topology/content/materialization revisions, unmount/remount retention, conflict/branch projection, local refresh flush, and two-stage locate cancellation.
- Consumer convergence: Reader/word count/whole copy/Bookmark Preparation/local selection/formula copy/export/Directory/Stepper all use the same V2 port; V1 objects are only projections over that port. Bookmark save requires one ready turn read with identity, canonical Markdown, and topology position.

The focused command is `npm run test:chatgpt-discovery`. It must be followed by the repository test ladder, `npm run type-check`, `npm run build`, and `git diff --check`. Installed Chrome MV3 and Firefox MV2 acceptance is recorded separately and is never inferred from Vitest/build output. The V2 runtime must remain free of cookies, storage-token/auth-header reads, conversation backend GET, POST/SSE construction, React internals, permanent polling, and synthetic scroll.
