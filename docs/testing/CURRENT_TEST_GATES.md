...
For ChatGPT content-discovery or conversation-root replacement changes, the real trigger path must prove `/` → temporary `/c/WEB:*` → canonical `/c/:id` publishes the completed first typed turn, then promotes it to a complete graph through one bounded same-origin read when passive capture is absent. If that read is unavailable, verified DOM evidence remains explicitly partial and Save Messages must reject it. A graph baseline still supplies every virtualized round; subsequent identity-overlapping DOM successors append once; a partial snapshot may only grow by strict semantic prefix; and virtualized shrink or branch conflict must retain last-good as stale rather than publish the smaller/divergent candidate. Coverage must prove resource completion matches a generation request observed by the bridge, stays bound to its request-start conversation, permits only blank-route creation to adopt the new canonical route, clears old completion evidence on the next start, and never reads request bodies, generation payloads, cookies, tokens, auth/session headers, or cloned SSE responses. PageIndex must ignore unrelated host child-list churn while continuing to observe conversation/message structure and locale-independent lifecycle `data-testid` changes without localized `aria-label` or `characterData` observation. ConversationIndex reads the bound Content Port directly and only projects current state to anchors; ambiguous identities and late route results fail closed; concurrent `refresh()` calls share one acquisition; and runtime disable→enable rebuilds the projection without duplicate subscriptions. Passive word-count, bookmark-active, Index, and Reader revision updates must not trigger a second acquisition; each user command may request at most one refresh through `ConversationContentSourceV1`. Reader exact-prefix extension appends once, canonical correction replaces atomically, Reader may retain stale last-good, and Save Messages only accepts `ready + complete`; delayed bookmark, Detached Reader, and annotation actions must reject a changed content token. Focused coverage must include `ConversationContentRepository`, bridge transport, `ConversationIndex`, `PageIndex`, DOM turn facts, discovery adapter/coordinator, materialization, directory controller, Reader binding, Reader/export/bookmark/toolbar entrypoints, content entry, and toolbar scheduler tests. Static boundary coverage must reject conversation/session credential reads, generation body/SSE parsing, endpoint cascades, consumer-side retry polling, a second content-discovery observer, consumer `setSnapshot`, UI/controller `ensureReady`, UI/controller `buildChatGPTReaderItems`, duplicate Reader content entrypoints, and ChatGPT DOM-tail discovery.
...
For ChatGPT direct atomic selection changes, focused verification must include strict partial/full coverage for every supported rendered unit, mixed complete-plus-partial sibling fail-open behavior, complete inner atoms inside partial structural ancestors, nested-unit collapse, KaTeX visual-layer coverage, reversible TeX extraction that differs from visual text, balanced outer-delimiter normalization, no-source formula fail-open behavior including formulas nested inside structural units, parser-budget fail-open behavior, atomic highlighting that remains valid when Markdown serialization fails, and one canonical Markdown snapshot shared by the configured keyboard exits. Snapshot coverage must prove the selected DOM is normalized exactly once, necessary formatting ancestors are re-closed around a partial native Range, ordered-list start/value semantics are retained, duplicate ChatGPT component blocks and host chrome are removed once, the native Range is not modified, and no normalized DOM clone remains in the completed snapshot. Formula parser coverage must prove shared source recovery, parse-error cleanup, accessible-source compatibility, and `null` rather than `outerHTML` when no source can be recovered. The real trigger path must cover document `selectionchange` → canonical snapshot/highlight → configured `keydown` handling, the `mod-c` copy finalizer, the `mod-shift-c` direct clipboard path, host handlers before and after the extension phase, runtime settings delivery, repeated init/dispose, no selection portal, i18n labels, third-party event propagation, editable-target guards, and shortcut invalidation when the Range changes. `none` must leave host copy untouched; `mod-c` must clear pre-existing clipboard types and win the final Markdown-only `text/plain` write without stopping propagation; `mod-shift-c` must prevent the page default action only for a valid user selection and use the shared text-only helper. Settings changes must swap shortcuts without disposing or duplicating listeners. Selection changes and either copy mode must not create HTML/MathML/Office Math payloads or load the Reader/export renderer graph. Partial, cross-message, streaming, no-source, over-budget, editable-target, DevTools-conflict, and unsupported selections must fail open. Historical `atomicMarkdownCopy` values must migrate to the new shortcut enum, while single-formula MathML asset copy remains covered separately. Run `tests/unit/core/latex/extractLatexSource.test.ts`, `tests/unit/drivers/content/adapters/parser/katex.test.ts`, `tests/unit/services/copy/copy-markdown.contract.test.ts`, `tests/unit/services/reader/atomicSelection.test.ts`, `tests/unit/services/copy/atomicSelectionMarkdown.test.ts`, `tests/unit/services/reader/readerMarkdownCopy.test.ts`, `tests/unit/ui/reader/readerPanel.atomicSelection.test.ts`, `tests/unit/drivers/content/clipboard/copyMathmlToClipboard.test.ts`, `tests/unit/ui/content/controllers/ChatGPTAtomicSelectionController.test.ts`, `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/governance/content-feature-boundary.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts`, and `tests/unit/governance/performance-benchmark-contract.test.ts`, then `npm run test:core`, `npm run test:smoke`, `npm run test:acceptance`, `npm run test:ui -- --run`, `npm run test:ui:visual`, `npm run build`, and the three-run `npm run perf:chatgpt` median. The performance fixture must select and copy a reversible-source formula, assert the final clipboard contains only canonical `text/plain`, observe no long task, prove the selection path does not request the content-feature or export renderer graph, and retain zero renderer requests before any image action.
