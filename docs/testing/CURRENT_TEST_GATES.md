...
For ChatGPT content discovery, directory, or bookmark-position changes, targeted verification must prove every entrypoint agrees on the same Engine semantic snapshot and ConversationIndex navigation projection, including full canonical count/order and typed identity:
...
For ChatGPT conversation-root replacement changes, the real mutation trigger path must prove the directory keeps the complete canonical count while anchors are replaced, an unknown typed anchor forces semantic refresh, and the toolbar mounts exactly once in the replacement root. Coverage must also prove PageIndex owns the shared mutation signal, ConversationIndex resolves a unique mounted assistant `data-message-id` to the canonical `assistantMessageId` even when host wrapper/turn IDs drift, ambiguous identities fail closed, a completed consecutive DOM successor updates the existing verified snapshot instead of creating a second consumer-specific source, and runtime disable→enable rebuilds and rebinds the projection. Focused coverage must include Engine, ConversationIndex, PageIndex, live DOM content, directory controller, message stepper, content entry, and toolbar scheduler tests.
...
For ChatGPT content-discovery changes, browser evidence is part of the gate, not an optional manual check. The only accepted semantic source is one verified same-conversation snapshot: a passively observed same-origin `GET` provides its complete graph baseline, and the shared PageIndex mutation signal may only complete the canonical pending tail in place or append a completed, non-empty, typed DOM successor whose immediate predecessor is the current canonical tail. A forced Reader/word-count refresh must synchronously flush a pending live reconciliation before returning, without initiating a conversation/session request. A later empty same-lineage graph must not regress DOM-proven completed content; a non-empty graph retakes content authority, while a conflicting assistant identity must switch to the graph branch. Multiple mounted successors must publish their completed contiguous prefix even when a later round remains incomplete. The extension must not observe generation POST/SSE, request a session/conversation, read Cookie/Token/authentication headers, request bodies, or construct authentication information. React props, internal stores and visible DOM are not history-completeness fallbacks. Invalid, incomplete, cyclic, stale-route, unknown-predecessor, identity-conflicting, or malformed graph/live-tail data must fail closed or preserve only the permitted verified same-conversation snapshot. ConversationIndex must preserve the complete Engine count while virtual windows change, resolve a remounted visible assistant by its unique canonical message identity despite host turn-wrapper drift, and materialize off-screen targets by exact typed identity. Toolbar full-scan coverage must prove every mounted assistant surface is inventoried independently from round pairing, so an empty virtualized user host slot cannot suppress the visible assistant's toolbar, canonical word count, or Reader trigger; the inventory must reuse the existing debounced observer lifecycle and add no observer, polling loop, or per-message listener. Navigation coverage must prove that a canonical target can calibrate and reuse its persistent sibling host slot without pixel probing, remains on that slot through slow long-distance hydration, normalizes nested wrappers that repeat the outer slot marker, never pairs user/assistant turns across an empty virtual slot, and does not report success for a disconnected or persistently drifting exact anchor. The normal trigger path must add no diagnostic whole-page observers; a repeated hydration attempt must perform one slot discovery query and zero `scrollRoot.scrollTo()` calls. A published Engine snapshot must update directory/Reader and recompute materialized toolbar word counts without another per-consumer semantic refresh. Focused coverage must include static no-auth/active-fetch guards plus `ChatGPTConversationEngine.test.ts`, `chatgptConversationBridge.test.ts`, `ChatGPTConversationIndex.test.ts`, `ChatGPTPageIndex.test.ts`, `chatgptLiveDomContent.test.ts`, `readerContentSource.test.ts`, `chatgptDirectory.navigation.test.ts`, directory/stepper real-trigger caller tests, toolbar/Reader mapping, lifecycle disable→enable, and a live long-conversation route/scroll/navigation check that includes a newly generated response plus first↔last, random, reverse, and repeated targets while recording exact message identity, final anchor tolerance, changing scroll height, and latency.

For ChatGPT direct atomic selection changes, focused verification must include strict partial/full coverage for every supported rendered unit, mixed complete-plus-partial sibling fail-open behavior, complete inner atoms inside partial structural ancestors, nested-unit collapse, KaTeX visual-layer coverage, reversible TeX extraction that differs from visual text, balanced outer-delimiter normalization, no-source formula fail-open behavior including formulas nested inside structural units, parser-budget fail-open behavior, and one canonical Markdown snapshot shared by plain and rich exits. Snapshot coverage must prove the selected DOM is normalized exactly once, necessary formatting ancestors are re-closed around a partial native Range, ordered-list start/value semantics are retained, duplicate ChatGPT component blocks and host chrome are removed once, the native Range is not modified, and no normalized DOM clone remains in the completed snapshot. Formula parser coverage must prove shared source recovery, parse-error cleanup, accessible-source compatibility, and `null` rather than `outerHTML` when no source can be recovered. Reader native selection copy and its selection action must both enter the shared canonical Markdown clipboard finalizer and honor the same formula setting. Rich coverage must prove it remains functional after page DOM removal, accepts only canonical Markdown plus final plain Markdown, uses the Reader GFM/math parser and sanitizer for headings, lists, tables, links, images, deleted text, code and quotes, emits inline/display formulas in all five `richCopyFormulaFormat` source wrappers, escapes HTML, handles multiple formulas, rejects unavailable source before snapshot completion, and never copies page MathML or raw host HTML. The real trigger path must cover document `selectionchange` → canonical snapshot/highlight → single formatted-copy action → lazy content-feature load → Markdown-AST-to-HTML serialization → clipboard write, plus the controller's existing window Markdown handler, host handlers before and after the extension phase, runtime settings delivery, repeated init/dispose, portal Escape/outside/scroll/resize lifecycle, i18n labels, third-party event propagation, and stale payload rejection when the native selection changes during lazy loading. Selection changes and keyboard Markdown copy must not prepare a rich payload or load the Reader/export renderer. A strict `Command/Ctrl+C` source copy must clear pre-existing clipboard types and win the final Markdown-only `text/plain` write without stopping propagation. Rich payloads must carry semantic `text/html` plus the snapshot's final Markdown `text/plain` representation, prefer `ClipboardItem`, use one-shot native copy only as a rich fallback, and never silently claim a text-only success. Unavailable source, serializer failure, unavailable copy-event data, rejected writes, or stale async results must not alter the independent keyboard Markdown path. Partial, cross-message, streaming, no-source, and unsupported selections must not take ownership. Old direct-rich `word-office-math` and `mathml` settings must normalize to Markdown dollar; these modes are not current acceptance targets, while single-formula MathML asset copy remains covered separately. Run `tests/unit/core/latex/extractLatexSource.test.ts`, `tests/unit/drivers/content/adapters/parser/katex.test.ts`, `tests/unit/services/copy/copy-markdown.contract.test.ts`, `tests/unit/services/reader/atomicSelection.test.ts`, `tests/unit/services/copy/atomicSelectionMarkdown.test.ts`, `tests/unit/services/copy/atomicSelectionRichHtml.test.ts`, `tests/unit/services/reader/readerMarkdownCopy.test.ts`, `tests/unit/ui/reader/readerPanel.atomicSelection.test.ts`, `tests/unit/drivers/content/clipboard/copyRichTextToClipboard.test.ts`, `tests/unit/drivers/content/clipboard/copyMathmlToClipboard.test.ts`, `tests/unit/ui/content/controllers/ChatGPTAtomicSelectionController.test.ts`, `tests/unit/runtimes/content/lazyContentFeatures.test.ts`, `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/governance/content-feature-boundary.test.ts`, `tests/unit/ui/components/ToolbarHoverActionPortal.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts`, and `tests/unit/governance/performance-benchmark-contract.test.ts`, then `npm run test:core`, `npm run test:smoke`, `npm run test:acceptance`, `npm run test:ui -- --run`, `npm run test:ui:visual`, `npm run build`, and the three-run `npm run perf:chatgpt` median. The performance fixture must select and copy a reversible-source formula, assert the final clipboard contains only canonical `text/plain`, observe no long task, prove the default Markdown-dollar path does not request the content-feature or export renderer graph, and retain zero renderer requests before any image action.
