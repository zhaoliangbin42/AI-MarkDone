...
For ChatGPT content-discovery or conversation-root replacement changes, the real trigger path must prove an empty new-conversation epoch reaches one ready first-round snapshot after streaming completes even when no graph capture occurs; a graph baseline still supplies every virtualized round; subsequent typed DOM successors append once; and regenerate identity conflict withdraws the old snapshot before export. Coverage must also prove PageIndex owns the single shared mutation signal with no `characterData` observation, ConversationIndex reads the bound Content Port directly and only projects current state to anchors, a unique mounted assistant `data-message-id` resolves to the canonical `assistantMessageId` even when host wrapper/turn IDs drift, ambiguous identities fail closed, route-epoch-late facts are ignored, concurrent `refresh()` calls share one acquisition, and runtime disable→enable rebuilds and rebinds the projection without duplicate subscriptions. Passive word-count, bookmark-active, Index, and Reader revision updates must not trigger a second acquisition; each user command may request at most one refresh through `ConversationContentSourceV1`. Reader exact-prefix extension appends once, canonical correction replaces atomically, and snapshot withdrawal closes Reader and Save Messages; bookmark writes returning from a dialog, Detached Reader refresh snapshot construction, and in-page annotation show/focus completion must reject a changed content token. Focused coverage must include `ConversationContentRepository`, bridge transport, `ConversationIndex`, `PageIndex`, DOM turn facts, discovery adapter/coordinator, materialization, directory controller, Reader binding, Reader/export/bookmark/toolbar entrypoints, content entry, and toolbar scheduler tests. Static boundary coverage must reject active conversation/session requests, generation POST/SSE observation, authentication data access, consumer-side retry polling, a second content-discovery observer, consumer `setSnapshot`, UI/controller `ensureReady`, UI/controller `buildChatGPTReaderItems`, duplicate Reader content entrypoints, and ChatGPT DOM-tail discovery.
