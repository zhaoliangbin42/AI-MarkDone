...
| ChatGPT message navigation | Directory rail retired; lightweight lower-right stepper is optional | ChatGPT 已提供官方对话导航，因此 AI-MarkDone 不再挂载自己的右侧目录条；`chatgptDirectory.enabled` 归一化为 `false`，Settings 仅显示下架说明。右下角轻量 stepper 默认显示上一条/下一条入口，可通过 `chatgptBehavior.showMessageStepper` 关闭；左右方向键导航由 `chatgptBehavior.enableArrowKeyMessageNavigation` 单独启用/关闭；Reader locate、Bookmark Go 与 pending navigation 继续复用同一条 ChatGPT same-page navigation helper。 |
...
| Conversation directory rail | Temporarily retired visible surface; Settings `chatgptDirectory` retained for storage compatibility | `src/ui/content/controllers/ChatGPTDirectoryController.ts`, `src/ui/content/chatgptDirectory/ChatGPTDirectoryRail.ts` | `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/ui/content/controllers/ChatGPTDirectoryController.test.ts`, `tests/unit/services/settings/settingsService.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts` | AI-MarkDone 当前不再挂载 ChatGPT 右侧目录条；`chatgptDirectory.enabled` 始终归一化为 `false`，Settings 仅展示下架说明，不再创建目录条开关、mode 或 prompt label controls。兼容字段仍可被迁移/归一化，代码保留为可恢复 surface，正文发现、Reader、导出、复制、书签存储不依赖它。 |
