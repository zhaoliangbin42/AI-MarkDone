...
| ChatGPT conversation directory | Temporarily retired as a visible surface; helper code remains for same-page positioning | ChatGPT 已提供官方对话导航，因此 AI-MarkDone 不再挂载自己的右侧目录条/step controls；`chatgptDirectory.enabled` 归一化为 `false`，Settings 仅显示下架说明；Reader locate、Bookmark Go 与 pending navigation 继续复用同一条 ChatGPT same-page navigation helper。 |
