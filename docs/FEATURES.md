...
| ChatGPT message navigation | Compact directory for 1-4 messages; lightweight lower-right stepper is optional | AI-MarkDone 挂载新的紧凑右侧目录条，且仅在当前 conversation 发现到 1-4 条消息时显示；`chatgptDirectory.enabled` 控制该新版目录是否启用。旧 `ChatGPTDirectoryController` / `ChatGPTDirectoryRail` 保留为历史可恢复代码但不挂载。右下角轻量 stepper 默认显示上一条/下一条入口，可通过 `chatgptBehavior.showMessageStepper` 关闭；左右方向键导航由 `chatgptBehavior.enableArrowKeyMessageNavigation` 单独启用/关闭；Reader locate、Bookmark Go 与 pending navigation 继续复用同一条 ChatGPT same-page navigation helper。 |
...
| Compact conversation directory rail | Optional right-side compact rail for 1-4 messages | `src/ui/content/controllers/ChatGPTCompactDirectoryController.ts`, `src/ui/content/chatgptDirectory/ChatGPTCompactDirectoryRail.ts`, legacy retained in `src/ui/content/controllers/ChatGPTDirectoryController.ts` / `src/ui/content/chatgptDirectory/ChatGPTDirectoryRail.ts` | `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/ui/content/controllers/ChatGPTCompactDirectoryRail.test.ts`, `tests/unit/services/settings/settingsService.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts` | 新版目录条只在发现 1-4 条消息时显示；折线 marker 宽度保持统一，当前消息只通过颜色区分，不再拉长当前条。hover/touch/focus 到目录任意位置都会打开一个整体浮层，显示全部标题；浮层固定最大高度，标题过多时内部滚动。Settings 只暴露 `chatgptDirectory.enabled` 开关，旧 mode / prompt label controls 不恢复。旧 directory rail/controller 代码不删除、不挂载。 |
...
| ChatGPT settings | Settings tab `ChatGPT Settings` → compact directory toggle + restore-position toggle + stepper toggle + arrow-key message navigation toggle | `src/core/settings/types.ts`, `src/services/settings/settingsService.ts`, `src/ui/content/bookmarks/ui/tabs/SettingsTabView.ts`, `src/runtimes/content/entry.ts` | `tests/unit/services/settings/settingsService.test.ts`, `tests/unit/runtimes/background/settings-handler.test.ts`, `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts` | `chatgptDirectory.enabled` 默认开启，并只控制新版 1-4 消息紧凑目录条；旧 directory mode / prompt label controls 不恢复。`chatgptBehavior.restorePositionAfterSend` 默认开启，并只控制 ChatGPT 发送后恢复阅读位置；`chatgptBehavior.showMessageStepper` 默认开启，并只控制右下角 Previous/Next 按钮显示；`chatgptBehavior.enableArrowKeyMessageNavigation` 默认开启，并只控制 Left/Right 键切换消息。 |
