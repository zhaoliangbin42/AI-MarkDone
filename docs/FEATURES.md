...
| Open detached Reader tab from the lower-right page controls | Lower-right Split View button → first-use experimental notice modal → confirm → fresh `readerContentSource` → `readerSession:create` → extension page `reader.html#sessionId=...` → fullscreen `ReaderPanel.show(...)` | `src/ui/content/controllers/ChatGPTMessageStepperController.ts`, `src/ui/content/controllers/MessageToolbarOrchestrator.ts`, `src/runtimes/background/handlers/readerSession.ts`, `src/runtimes/reader/entry.ts`, `src/services/reader/readerSessionSnapshot.ts` | `tests/unit/runtimes/background/readerSession-handler.test.ts`, `tests/unit/services/reader/readerSessionSnapshot.test.ts`, `tests/unit/contracts/protocol.test.ts`, `tests/unit/ui/content/controllers/ChatGPTMessageStepperController.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts` | Detached Reader 入口必须是右下角 page-control cluster 内的 icon-only Split View 按钮，位于 Previous/Next 左侧，不放在 per-message toolbar。首次点击先显示复用现有 modal/changelog notice 风格的实验性功能提示，说明它会把 Reader 打开到独立扩展页以绕开 ChatGPT 官方页渲染卡顿、仍需源 ChatGPT tab 执行刷新/发送/定位、源页不可达时只能保留快照、v1 不做实时同步或强保活；modal 提供“确定跳转”和“取消返回”，确认状态写入 settings/storage，取消不创建 session。每个 detached Reader 只连接自己的源 ChatGPT tab；多个 ChatGPT tabs 并发时按 `sessionId + sourceTabId + readerTabId` 隔离。独立页复用现有 Reader 渲染与基础能力，默认以 fullscreen Reader 打开；refresh/send/locate 经 runtime protocol 回源页执行。关闭源 ChatGPT tab 会 best-effort 关闭对应 Reader tab，关闭 Reader tab 不影响源页。 |
| Reader default open size and resizing | Settings `Reader` → default open size: `fullscreen` or `panel`; Reader panel resize handle persists page-session width when in panel mode | `src/core/settings/types.ts`, `src/ui/content/bookmarks/ui/tabs/SettingsTabView.ts`, `src/ui/content/reader/ReaderPanel.ts`, `src/runtimes/reader/entry.ts` | `tests/unit/services/settings/settingsService.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts`, `tests/unit/ui/reader/readerPanel.presentation.test.ts`, `tests/integration/reader/reader-panel.test.ts` | Settings 新增 Reader 默认打开大小，默认值为 `fullscreen`，可切换为半屏/面板模式。Fullscreen 用于 Detached Reader 独立页的默认体验，Reader surface 占满扩展页可视区域，不居中成半屏卡片。半屏模式允许通过现有 panel shell 增加拖拽调整宽度；拖拽只改变 Reader shell 宽度，不改变 Markdown 渲染、正文 source、分页、复制、发送、评论或 Sticky 数据合同；宽度必须 clamp 在可视区内并使用 `--aimd-*` tokens 表达 visual affordance。该 resizing 是 Reader surface 的内部能力，不允许调用方传 CSS 覆盖。 |
