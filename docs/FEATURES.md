...
| Reader content source | `readerContentSource -> ChatGPTConversationEngine snapshot -> ReaderPanel.show()` | `src/services/reader/readerContentSource.ts`, `src/services/reader/chatgptReaderItems.ts`, `src/ui/content/controllers/MessageToolbarOrchestrator.ts` | `tests/unit/services/reader/readerContentSource.test.ts`, `tests/unit/drivers/content/chatgpt/chatgptReaderItems.test.ts`, `tests/unit/ui/reader/readerPanel.bookmarkAction.test.ts`, `tests/unit/ui/content/messageToolbarOrchestrator.fold-action.test.ts` | Reader 在 ChatGPT 上优先消费 `ChatGPTConversationEngine` 的完整 snapshot（payload first，随后结构化 turn 数据），避免被当前 DOM hydration/virtualization 范围截断；snapshot 不可用时才 fallback 到 DOM `collectReaderItems()`。打开 ChatGPT Reader 前会强制刷新一次 snapshot；Reader 内仅最后一条 ChatGPT item 使用 live tail content provider，切换回该页时重新 force refresh，其他页保持打开时的快照固化；工具栏 Copy/Copy PNG 复用同一份 `ReaderItem` 语义并按需预热当前消息；刷新失败时回退到原始 ReaderItem 内容。 |
...
| Conversation directory rail | ChatGPT runtime → `ChatGPTDirectoryController`; Settings `chatgptDirectory` | `src/ui/content/controllers/ChatGPTDirectoryController.ts`, `src/ui/content/chatgptDirectory/ChatGPTDirectoryRail.ts` | `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/ui/content/controllers/ChatGPTDirectoryController.test.ts`, `tests/unit/services/settings/settingsService.test.ts`, `tests/unit/ui/bookmarks/settingsTabView.test.ts` | 右侧目录条按完整用户轮次展示历史；默认开启并保持 compact preview 模式；Settings 可单独隐藏目录条或切换 expanded list 模式；expanded 模式 hover/focus 展开全量用户消息条目、hover 高亮、单条标题默认最多显示开头 15 字符，也可切换为前 15 字符 + 省略号 + 后 15 字符；已收藏消息复用 `BookmarksPanelController` 的当前 URL position 集合，并在目录 marker 上显示 tokenized rainbow bookmark marker，不新增第二套书签查询链路；expanded 宽度由 CSS 字符宽度预算决定，避免固定像素宽度提前二次截断；目录很长时允许垂直滚动，expanded 行必须用明确的编号/文案/短线列布局和稳定 scrollbar gutter，hover/active 条目不得被 scrollbar 挤压裁切；hover accordion 与 compact preview 属于 rail UI 内部热路径，必须使用已渲染条目/轮次缓存与有限邻近状态更新，不得在鼠标移动时全量扫描 rail item、重写 body-level token style、读取布局定位或触发目录发现链路；click 在同页跳转到对应轮次；用户滚动页面时，active 高亮按整轮 user/assistant DOM 范围判定，并在用户未操作目录时让 rail 自身跟随 active 条目；上一条/下一条消息按钮是同一 controller 拥有的 body-level step controls，视觉上与 rail 分离但复用相同 active position 与 directory navigation helper；目录条设置不复用 `platforms.chatgpt`，关闭目录条不影响 Reader locate、书签 Go 或跨页 pending navigation；目录默认被动消费已缓存 snapshot，只有当 DOM 与缓存 snapshot 都仍给出低质量 `Message N` 标题时，才做一次有界 on-demand snapshot 补齐；DOM mutation 合并为真实 turn 结构变化后的索引重建，插件自身 DOM/toolbar 注入不触发目录全量重建；跳转不得抢占页面焦点或阻止用户继续浏览。 |
| ChatGPT viewport resize suspend | `ViewportResizeSuspendController -> data-aimd-viewport-resizing -> aimd:viewport-resize-idle` | `src/ui/content/controllers/ViewportResizeSuspendController.ts`, `src/runtimes/content/entry.ts`, `src/ui/content/controllers/ChatGPTDirectoryController.ts` | `tests/unit/ui/content/controllers/ViewportResizeSuspendController.test.ts`, `tests/unit/runtimes/content/entry.test.ts`, `tests/unit/ui/content/controllers/ChatGPTDirectoryController.test.ts` | ChatGPT viewport 宽度持续 resize 时，AI-MarkDone 目录 rail、preview、step controls 与 action-row message toolbar chrome 可以短暂隐藏并暂停子树渲染，让官方页面优先完成布局；该行为只由浏览器 resize + 宽度阈值驱动，不依赖 ChatGPT DOM/mutation；停止 resize 后恢复插件 chrome 并只补算一次目录 active position，不卸载 DOM、不重建 toolbar record、不折叠 action-row toolbar 布局、不改变目录、Reader、Save Messages 或书签数据合同。 |
