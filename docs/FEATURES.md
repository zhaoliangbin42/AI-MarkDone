...
| Injection algorithm | MO as signal + debounced scan + ChatGPT message lifecycle reconcile + route rebind | SPA/React 更稳定；message 可先出现、官方 action bar 后补出现，ChatGPT 可归属的 action-row hydration 只推进对应消息状态，整页重扫仅保留给 init、route change、observer rebind 或无法归属的异常结构。 |
