...
本文件保留 rewrite 阶段的历史执行方式与验收思路，供后续治理和历史追溯使用。当前主线代码已经完成对 `archive/` 的替换，后续工作重点转向治理门禁、文档一致性与过渡边界收口。

状态标签：

- `historical`：仅为历史过程说明
- `manual`：需要人工验证
- `moved`：已迁移到其它权威文档或自动化门禁
- `pending`：仍未收官
...
## 2. Rewrite 的工作方式（Archive → Rebuild → Delete，历史过程）
