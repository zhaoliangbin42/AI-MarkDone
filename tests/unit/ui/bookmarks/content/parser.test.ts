...
describe('bookmarks content parser', () => {
    it('parses changelog markdown for both locales', () => {
        ...
        expect(JSON.stringify(zh.entries[0]?.leadBlocks)).toContain('英雄召集贴');
        expect(JSON.stringify(zh.entries[0]?.leadBlocks)).toContain('缓存中对话的存放位置');
        ...
        expect(JSON.stringify(en.entries[0]?.sections[0]?.blocks)).toContain('page width not being restored');
