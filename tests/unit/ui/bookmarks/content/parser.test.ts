...
describe('bookmarks content parser', () => {
    it('parses changelog markdown for both locales', () => {
        ...
        expect(zh.entries.map((entry) => entry.version)).toEqual(['5.0.2', '5.0.1', '5.0.0', '4.8.0', '4.7.0', '4.6.0', '4.5.1', '4.5.0', '4.4.6', '4.4.5', '4.4.1', '4.4.0', '4.3.1', '4.3.0', '4.2.3', '4.2.2', '4.2.1', '4.2.0', '4.1.2', '4.1.1', '4.1.0', '4.0.0', '3.0.0']);
        expect(en.entries.map((entry) => entry.version)).toEqual(['5.0.2', '5.0.1', '5.0.0', '4.8.0', '4.7.0', '4.6.0', '4.5.1', '4.5.0', '4.4.6', '4.4.5', '4.4.1', '4.4.0', '4.3.1', '4.3.0', '4.2.3', '4.2.2', '4.2.1', '4.2.0', '4.1.2', '4.1.1', '4.1.0', '4.0.0', '3.0.0']);
        expect(zh.entries[0]?.date).toBe('2026-07-26');
        expect(zh.entries[4]?.date).toBe('2026-06-28');
        expect(en.entries[10]?.leadBlocks[0]).toEqual(
        ...
        expect(zh.entries[2]?.sections.map((section) => section.heading)).toEqual(zhFiveZeroSections);
        expect(en.entries[1]?.sections.map((section) => section.heading)).toEqual(enFiveZeroSections);
        expect(en.entries[2]?.sections.map((section) => section.heading)).toEqual(enFiveZeroSections);
        expect(zh.entries[1]?.sections[1]?.blocks).toContainEqual({
            type: 'image',
            alt: '好友迹——好友地图通讯录',
            src: 'icons/mappamory-promo-poster.png',
        });
        expect(zh.entries[4]?.sections[0]?.blocks).toContainEqual({
            type: 'image',
            alt: '好友迹 - 好友地图通讯录',
