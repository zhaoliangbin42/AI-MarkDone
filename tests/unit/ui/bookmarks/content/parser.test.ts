...
describe('bookmarks content parser', () => {
    it('parses changelog markdown for both locales', () => {
        ...
        expect(zh.entries.map((entry) => entry.version)).toEqual(['4.4.0', '4.3.1', '4.3.0', '4.2.3', '4.2.2', '4.2.1', '4.2.0', '4.1.2', '4.1.1', '4.1.0', '4.0.0', '3.0.0']);
        expect(en.entries.map((entry) => entry.version)).toEqual(['4.4.0', '4.3.1', '4.3.0', '4.2.3', '4.2.2', '4.2.1', '4.2.0', '4.1.2', '4.1.1', '4.1.0', '4.0.0', '3.0.0']);
        expect(zh.entries[0]?.date).toBe('2026-05-12');
        expect(en.entries[0]?.leadBlocks[0]).toEqual(
            expect.objectContaining({
                type: 'paragraph',
                text: expect.stringContaining('Export as PNG'),
            }),
        );
        expect(zh.entries[0]?.sections.map((section) => section.heading)).toEqual(['原理分享', '变更']);
        expect(en.entries[0]?.sections.map((section) => section.heading)).toEqual(['How it works', 'Changed']);
    });
