...
describe('SponsorTabView', () => {
    it('renders payment QR cards first, then GitHub support, then sponsor thanks', () => {
        ...
        expect(thanksList?.querySelectorAll('.sponsor-thanks-item').length).toBe(25);
        ...
        expect(root.textContent).toContain('Austin（希望能一直更新下去），回：好的，没问题，一定会继续更新');
        expect(root.textContent).toContain('Core（念即观）');
        expect(root.textContent).toContain('WhEaT');
        expect(root.textContent).toContain('wu（太感谢了，插件越来越好用了）');
