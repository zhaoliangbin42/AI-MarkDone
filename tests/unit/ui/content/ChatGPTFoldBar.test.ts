...
describe('ChatGPTFoldBar', () => {
    it('uses a flat single-line summary row with theme-aware surface, border, and hover feedback', () => {
        ...
        expect(style).toContain('--_foldbar-min-height: calc(var(--aimd-size-icon-md) + var(--aimd-space-2));');
        expect(style).toContain('white-space: nowrap;');
        expect(style).not.toContain('-webkit-line-clamp: 2;');
        expect(style).toContain('--_foldbar-surface: var(--aimd-bg-primary);');
        expect(style).toContain('border: 1px solid color-mix(in srgb, var(--aimd-border-default) 76%, var(--aimd-interactive-primary));');
        expect(style).toContain('.bar:hover {');
        expect(style).toContain('background: color-mix(in srgb, var(--_foldbar-surface) 82%, var(--aimd-interactive-hover));');
        expect(style).toContain('border-color: color-mix(in srgb, var(--aimd-border-default) 62%, var(--aimd-interactive-primary));');
        expect(style).toContain('.bar:active {');
        expect(style).toContain('background: color-mix(in srgb, var(--_foldbar-surface) 78%, var(--aimd-interactive-active));');
        expect(style).toContain('width: var(--aimd-size-icon-md);');
        expect(style).toContain('height: var(--aimd-size-icon-md);');
    ...
    it('uses a theme-aware compact dock surface with shorter buttons', () => {
        ...
        expect(style).toContain('background: var(--aimd-bg-primary);');
        expect(style).toContain('.btn {');
        expect(style).toContain('min-height: calc(var(--aimd-space-3) * 3);');
        expect(style).toContain('.btn:hover { background: var(--aimd-interactive-hover); }');
    });
