...
export class ChatGPTAdapter implements IPlatformAdapter {
    ...
    extractLatex(mathNode: HTMLElement): LatexResult | null {
        ...
        for (const strategy of strategies) {
            try {
                ...
                logger.warn('[AI-MarkDone][ChatGPTParserAdapter] LaTeX strategy failed', strategy.name, error);
            }
        }

        logger.error('[AI-MarkDone][ChatGPTParserAdapter] All LaTeX strategies failed');
    ...
    private extractFromMathML(mathNode: HTMLElement): LatexResult | null {
        ...
        try {
            ...
            logger.warn('[AI-MarkDone][ChatGPTParserAdapter] MathML parse failed', error);
            return null;
        }
    }
    ...
    private validateLatex(latex: string): boolean {
        ...
        if (latex.length > 10000) {
            logger.warn('[AI-MarkDone][ChatGPTParserAdapter] LaTeX too long', latex.length);
            return false;
        }

        if (latex.includes('<script>')) {
            logger.error('[AI-MarkDone][ChatGPTParserAdapter] XSS attempt detected in LaTeX');
            return false;
        }
