...
export const localStoragePort = {
    async get(keys: StorageKeys = null): Promise<Record<string, unknown>> {
        const result = await browser.storage.local.get(keys as any);
        return (result || {}) as Record<string, unknown>;
    },

    async set(patch: Record<string, unknown>): Promise<void> {
        await browser.storage.local.set(patch as any);
    },
    ...
    async getBytesInUse(keys: null | string | string[] = null): Promise<number> {
        const area: any = browser.storage.local as any;
        const fn = area?.getBytesInUse;
        if (typeof fn === 'function') {
            try {
                return await fn.call(area, keys);
            } catch (err) {
                logger.warn('[AI-MarkDone][StoragePort] getBytesInUse failed, falling back to estimate:', err);
            }
        }
