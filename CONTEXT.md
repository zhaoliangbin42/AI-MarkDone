...
- **Canonical conversation snapshot**: the one immutable Content Port V1 value that owns conversation identity, branch/order, typed turn identities, canonical assistant Markdown, coverage, and per-body provenance. Consumers never merge graph and DOM evidence themselves.
- **Conversation Content Port V1**: the stable `read / subscribe / refresh / isCurrent` Interface implemented by `ConversationContentRepository`. It publishes explicit `idle / syncing / ready / stale / unavailable` states and never exposes DOM, selectors, provider payloads, transport, or route epochs.
- **Content Source Adapter**: the platform driver that decodes provider evidence, normalizes provider Markdown dialect, attaches provenance, and submits only complete Source Graph candidates to the repository. Host DOM is never a body or position fallback.
- **Source provenance**: `{ authority, fidelity, producer }` attached to canonical Markdown. ChatGPT canonical content is source-backed; a missing or ambiguous source is unavailable rather than reconstructed.
- **Coverage**: a compatibility projection of the proof. ChatGPT consumers receive a complete sealed snapshot or unavailable; an unfinished tail is not published as a partial body.
...
- **Reader content source status**: the additive `ready / stale / unavailable / target-unresolved` state plus coverage and source quality. ChatGPT copy/bookmark/export exits require a sealed canonical turn and reject unresolved or reconstructed content.
...
- **Conversation Reader binding**: the source-only subscription that keeps the in-page Reader aligned with published revisions. It never discovers DOM turns or starts a second acquisition path.
...
- **Reconcile**: the single coalesced coordinator entry point for route, `pageshow`, PageIndex lifecycle, passive bridge, generation, and explicit refresh signals. It owns one document epoch, one in-flight acquisition, and stale-result rejection; it does not run a content timer.
