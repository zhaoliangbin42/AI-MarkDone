...
- **Host Monitor**: the single ChatGPT PageIndex-backed DOM input. After a 400 ms quiet window it compiles only newly observed, typed, completed turns and appends them to the cache. DOM is an incremental input, not a second consumer-facing content store. If virtualization leaves an already-cached assistant response mounted while its user node is temporarily detached, PageIndex preserves an assistant-only surface projection for materialization, toolbar mounting, and viewport geometry; that projection never creates a new cache turn.
...
- **Materialization**: the host-controlled act of mounting a typed conversation turn into the current DOM window. It may add or remove complete group anchors or an assistant-only surface projection, but it never changes canonical content or order. An assistant-only projection is valid only when it joins an already-cached assistant identity; it is not a DOM history fallback.
...
- **Conversation Index**: the unique navigation projection that combines canonical order with optional complete or assistant-only materialized anchors. Directory, stepper, locate, and bookmark navigation use it without depending on body parsing.
